...
import { handlePurchase, submitRequire, queryone, getQueryName, getQueryOddNumbers,getQueryQuantityAndUnit,querySubcontracto,querySubcontractoEndDate,queryMaterialAndRun,formSupTypeArray,formSupLevelArray } from '@/api/computation/equipFee'
...
export default {
  ...
  data () {
    return {
      ...
      loadTable: parameter => {
        ...
        if (!this.$store.state.menu_key) {
            parameter.creator_org_id = this.$store.state.menu_key
        }
  ...
  watch: {
    $route (to, from) {
      this.deductionDateFlag = true
      if (to.path === '/computation/equipFeeEdit') {
  ...
  methods: {
    ...
    handleChangeDeductionStartDate (date, dateString) {   
        ...
        const that = this
        // if (this.deductionDateFlag) {
        //   return
        // }
        const subcontractor_code = this.form.getFieldValue('subcontractor_code')
        ...
        if (subcontractor_name !== ''  && deduction_end_date !== undefined && deduction_end_date !== null && date !== null) {
            // 获取分包商最新核算的结束日期
            queryMaterialAndRun({
    ...
    handleChangeDeductionEndDate (date, dateString) {   
      ...
      if (subcontractor_name !== '' && deduction_start_date !== undefined  && date !== null) {
            queryMaterialAndRun({
                subcontractor_code: this.subcontractor_code,
    ...
    loadEditInfo (res) {
        ...
        if (res.id !== undefined) {
          ...
          this.infoId = res.responseObject.id
        }
        if (res.responseList == undefined && res.responseObject !== null){
            ...
            formData.create_date = moment(res.create_date)
            if (res.deduction_start_date !== undefined) {
              formData.deduction_start_date = moment(res.deduction_start_date)
            } else {
              this.isDeductionStartDate = false
            }
            if (res.deduction_end_date !== undefined) {
                formData.deduction_end_date = moment(res.deduction_end_date)
            } 
            this.subcontractor_code = res.subcontractor_code
            form.setFieldsValue(formData)
        } else {
          if (res.responseObject !== null && res.responseList.length > 0) {
          ...
          if (that.params) {
            ...
            if (type === 'submit') {
              ...
              this.$confirm({
                ...
                onCancel () {that.submitLoading = false }
              })
            } else {

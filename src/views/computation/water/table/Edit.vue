<template>
  <div>
    <!-- 基本信息 -->
    <a-card class="card" title="水电费用扣款基本信息" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
    ...
    >
      <div class="attachment" v-if="activeTabKey === 'picture'">
        <a-upload-dragger
          ...
          :data="{ model: mod, type: 0 ,menu_id:67}"
        >
...
import { handlePurchase, submitRequire, queryone, getQueryName, getQueryOddNumbers,getQueryQuantityAndUnit,querySubcontracto,querySubcontractoEndDate,queryWaterAndelectricity,formSupTypeArray,formSupLevelArray } from '@/api/computation/water'
...
export default {
  ...
  watch: {
    $route (to, from) {
      ...
      if (to.path === '/computation/waterFeeEdit') {
  ...
  methods: {
    ...
    handleChangeDeductionEndDate (date, dateString) {   
      ...
      if (subcontractor_name !== ''&& deduction_start_date !== undefined && deduction_start_date !== null  && date !== null) {
            queryWaterAndelectricity({
                subcontractor_code: this.subcontractor_code,

...
import { handlePurchase, exportToExcel, exportToPDF, formStatusLabel, formStatusArray } from '@/api/computation/water'
...
export default {
  ...
  watch: {
    $route (to, from) {
      if (from.path === '/computation/waterFeeNew' || from.path === '/computation/waterFeeEdit') {
        this.refresh(true)
      }
    }
  },
  ...
  methods: {
    ...
    onChangeTime(date,range2){
      this.queryParam.begin_acceptance_date = range2[0]
      this.queryParam.end_acceptance_date = range2[1]
    },
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'waterFeeEdit', params: this.selectedRows[0] })   
      } else {
        return this.$router.push({ name: 'waterFeeNew',params: { type: 1 } })
      }
    },
    handleOk () { },
    ...
    exportTable (e) {
      ...
      if(e.key === 'export-excel') {
        ...
        exporToPdf ({ flag: 0, details:  this.selectedRowKeys  })
        .then(res => {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handlePurchase({ details: arrkeys,menu_id:67}, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk() {
          ...
          return handlePurchase({ details: arrkeys, flag: 0, menu_id: 67 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handlePurchase({ details: that.selectedRowKeys, menu_id: 67 }, 'cancel').then(res => {

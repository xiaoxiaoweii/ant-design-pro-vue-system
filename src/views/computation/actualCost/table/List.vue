<template>
  <div>
    ...
    <s-table
      ...
      :pagination="{showSizeChanger: true, showQuickJumper: true,pageSizeOptions: ['13', '26', '39'],  showTotal: total => `共 ${total} 条`}"
      bordered
    >
...
import { handlePurchase,exportToExcel, formStatusArray, exportToPDF} from '@/api/computation/actualCost'
      ...
      loadData: parameter => {
        ...
        return handlePurchase(Object.assign(parameter, this.queryParam), 'retrieve').then(res => {
          res.responsePageInfo.list = res.responsePageInfo.list.map(x => {
            x.equip_cost = (x.equip_cost== null ? '': x.equip_cost.toFixed(2) ) 
            return x
          })
  ...
  methods: {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        content: '确定取回已选择的记录?',
        onOk () {
           return handlePurchase({ids: that.selectedRowKeys ,menu_id:11 }, 'cancel').then(res => res.status === '0' && that.refresh(true))
        },
        onCancel () { }
      })
    },

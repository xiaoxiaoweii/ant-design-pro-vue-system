<template>
  <div>
    ...
    <s-table
      ...
      bordered
    >
      <a slot="form_number" slot-scope="text" @click="clickEdit(text)" href="javascript:;">{{ text }}</a>
      <!-- 新搜索页面 -->
      <div slot="filterDropdown" slot-scope="{ setSelectedKeys, selectedKeys, confirm, clearFilters,column}" class='custom-filter-dropdown'>
        {{ setFilterColumnScope({ setSelectedKeys, selectedKeys, confirm, clearFilters, column }) }}
        <a-range-picker
          v-if="column.dataIndex==='create_date'||column.dataIndex==='approvalDate'||column.dataIndex==='time'"
          v-model="range "
          @change="onChange"
...
import { handlePurchase, exportToExcel, exportToPDF, formStatusArray} from '@/api/computation/equipCost'
...
export default {
  ...
  computed: {
    ...
    columns () {
      ...
      return  [
        ...
        },
        {
          ...
          dataIndex: 'form_number',
           scopedSlots: {
            ...
            customRender: 'form_number',
          },
          onFilter: (value, record) => record.form_number.toLowerCase().includes(value.toLowerCase()),
          ...
          sorter: (a, b) => a.form_number.length - b.form_number.length
        },
        {
          title: '编制人',
        ...
        {
          title: '设备成本预算金额（元）',
          dataIndex: 'total_equip_budget_amount'
        },
        {
          title: '开累实耗设备成本（元）',
          dataIndex: 'total_actual_equip_cost'
        },
        {
          title: '状态',
      ...
      loadData: parameter => {
        ...
        return handlePurchase(Object.assign(parameter, this.queryParam), 'retrieve').then(res => {
          res.responsePageInfo.list = res.responsePageInfo.list.map(x => {
            x.start_time = (x.start_time == null ? '': x.start_time ) 
            x.end_time = (x.end_time == null ? '': x.end_time )
            x.total_equip_budget_amount = (x.total_equip_budget_amount == null ? '': x.total_equip_budget_amount.toFixed(2) ) 
            x.total_actual_equip_cost = (x.total_actual_equip_cost == null ? '': x.total_actual_equip_cost.toFixed(2) ) 
            return x
          })
  ...
  methods: {
    ...
    clickEdit (text) {
      const arr = this.dataSource.filter(x => x.form_number === text)
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        content: '确定取回已选择的记录?',
        onOk () {
          return handlePurchase({ ids: that.selectedRowKeys , menu_id:12 }, 'cancel').then(res => res.status === '0' && that.refresh(true))
        },
        onCancel () { }
      })
    },

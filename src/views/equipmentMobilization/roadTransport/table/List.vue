...
import { handlePurchase, exportPDF, exportPDFList, exportExcel, exportExcelList } from '@/api/equipmentMobilization/roadTransport'
...
export default {
    ...
    columns () {
      ...
      return [
        ...
        },
        {
          ...
          onFilter: (value, record) => record.updated_at,
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
  ...
  data () {
    return {
      ...
      orgparam: {
        menu_id: 37,
        scope_code: this.$store.state.menu_key || this.$store.state.first_key
      },
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'roadTransportEdit', params: this.selectedRows[0] })
      } else {
        return this.$router.push({ name: 'roadTransportNew', params: { type: 1 } })
      }
    },
    handleRead () {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handlePurchase({ details: that.selectedRowKeys, flag: 0, menu_id: 37 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handlePurchase({ details: that.selectedRowKeys, flag: 0, menu_id: 37 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      if (this.selectedRows.length === 0) return this.noSelect()
      const that = this
      console.log(this.selectedRows, '选择的数据')
      if (this.selectedRows.length === 1) {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handlePurchase({ details: that.selectedRowKeys, menu_id: 37 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

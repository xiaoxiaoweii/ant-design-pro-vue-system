  ...
  <div>
    ...
    >
      ...
      >
        ...
        <a-range-picker
          v-if="column.dataIndex === 'accept_date'"
          v-model="range"
          @change="onChangeDate"
...
import { handlePurchase, exportPDF, exportPDFList, exportExcel, exportExcelList } from '@/api/equipmentMobilization/siteAccept'
      ...
      return [
        ...
        },
        {
          ...
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'equip_type'
          },
          onFilter: (value, record) => record.equip_type.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
        ...
        },
        {
          ...
          onFilter: (value, record) => record.accept_date.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
          ...
          sorter: (a, b) => a.accept_date.split('-').join('') - b.accept_date.split('-').join('')
        },
        {
          title: '验收结果',
          dataIndex: 'result',
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'siteAcceptEdit', params: this.selectedRows[0] })
      } else {
        return this.$router.push({ name: 'siteAcceptNew', params: { type: 1 } })
      }
    },
    handleRead () {
    ...
    onChangeDate (date, range) {
      this.queryParam.begin_accept_date = range[0]
      this.queryParam.end_accept_date = range[1]
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handlePurchase({ details: that.selectedRowKeys, flag: 0, menu_id: 33 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handlePurchase({ details: that.selectedRowKeys, flag: 0, menu_id: 33 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handlePurchase({ details: that.selectedRowKeys, menu_id: 33 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

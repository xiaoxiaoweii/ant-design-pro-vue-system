<template>
  <div>
          ...
          <!-- <a-col :span="8">
            <a-form-item read-only label="单位" v-bind="formItemLayout">
              <a-input
                ...
                v-decorator="[
                  'unit',
                  { rules: [{ maxlength: 32, message: '内容未输入或超过字符长度限制',required: isrequired }] }
                ]"
              />
            </a-form-item>
          </a-col>-->
    ...
    <!-- 验收人HR树 -->
    <div>
      <a-modal title="请选择" v-model="visible_true" width="70%" @ok="handleNameOk(selectedRowKeys)">
        <a-row :gutter="8">
          <a-col :span="8">
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return queryByEquipment(Object.assign(parameter, this.queryParam, { menu_id: 33 })).then(res => {
          // 索引
  ...
  methods: {
    ...
    change (pagination, filters, sorter) {
      console.log(pagination, filters, sorter, '211111111111111111111111111')
    ...
    handleOk (val) {
      ...
      getQueryCheckItem({
        ...
        if (res.status === '0' && res.responseList) this.acceptData = res.responseList.map((d, i) => {
          d.check_item = d.inspect_name || '--'
          d.content = d.inspect_content || '--'
          d.result = '良好'
          d.remark = '测试默认'
    ...
    async loadEditInfo (data) {
      ...
      if (data.id) {
        await queryone({ id: data.id, menu_id: 33 }).then(res => {
          ...
          this.equipment_category_code = data.equipment_category_code
          // this.value = res.responseObject.equip_source || '调拨单号'
          if (res.responseObject.equip_source === '租赁设备') {
            this.leaseVisible = true
          }
          this.number = true
          ...
          this.equipData = res.responseObject.details2.map((d, i) => {
            d.check_expire_date ? d.check_expire_date1 = moment(d.check_expire_date) : null
        ...
        formData.create_date = data.create_date ? moment(data.create_date) : null
        formData.purchase_date = data.purchase_date ? moment(data.purchase_date) : null
        formData.in_date = data.in_date ? moment(data.in_date) : null
        formData.out_date = data.out_date ? moment(data.out_date) : null
        formData.production_time = data.production_time ? moment(data.production_time) : null
        formData.accept_date = data.accept_date ? moment(data.accept_date) : null
        // formData.purchase_date = data.purchase_date == null ? delete data.purchase_date : moment(data.create_date)
    ...
    saveOrSubmit (type) {
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          ...
          values.accept_to_person = that.params.accept_to_person
          values.menu_id = 33

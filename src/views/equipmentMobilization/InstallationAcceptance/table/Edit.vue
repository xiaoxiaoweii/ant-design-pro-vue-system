...
export default {
  ...
  methods: {
    ...
    showTable () {
      ...
      this.$refs.Ctables && this.refresh('Ctables')
    },
    handleName (val) {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryone({ id: data.id, menu_id: 36 }).then(res => {
          ...
          this.equip_code = data.equip_code
          this.number = true
          ...
          const formData = pick(data, ['creator_org_id', 'check_result', 'check_remarks', 'serial_number', 'pro_unit', 'creator_user_name', 'create_date', 'check_org', 'contract_no', 'install_org', 'fee_with_tax', 'manage_code', 'equip_name', 'spec', 'check_date', 'install_person', 'check_person'])
            ...
            if (type === 'submit') {
              this.$confirm({
                ...
                onOk () {
                  ...
                  //   x.content = x.inspect_name + '丨' + x.check_item
                  // })

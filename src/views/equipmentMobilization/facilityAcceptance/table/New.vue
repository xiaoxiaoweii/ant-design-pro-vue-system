...
export default {
  ...
  data() {
    return {
      ...
      popFields: ['name'],
      inputFields: ['source', 'spec', 'check_remark', 'name', 'remark', 'product_org', 'use_site', 'use_org', 'num'],
      // input_resultFields:[],
      ...
      loadTable: parameter => {
        ...
        return queryAllContract(Object.assign(parameter, this.queryParam, { menu_id: 36 })).then(res => {
          // 索引
  ...
  watch: {
    $route(to, from) {
      ...
      if (from.path === '/equipmentMobilization/facilityAcceptanceList') {
  ...
  methods: {
    ...
    handleUser() {
      ...
      console.log('选择数据', arr)
      console.log('回填数据', this.clickObj)
      this.detailData.forEach((x, i) => {
        if (x.key === this.clickObj.pid) {
          console.log(x, i)
          x.waterElectricityDetails.forEach((d, n) => {
            console.log(d, n)
            if (d.key === this.clickObj.key) {
    ...
    newDevice () {
      if (!this.ipValue) return this.noSelect('请先选择安装工程！')

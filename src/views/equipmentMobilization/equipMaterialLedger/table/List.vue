  ...
  <div>
    ...
    >
      ...
      >
        ...
        <a-range-picker
          v-if="column.dataIndex === 'material_date'"
          v-model="range"
          @change="onChangeDate"
        ...
        <a-button
          ...
          icon="search"
          size="small"
          :style="`width: 90px; margin-right: 8px; margin-left: ${column.dataIndex === 'material_date' ? 15 : 0}px`"
        >查询</a-button>
        <a-button
...
import { handlePurchase, exportPDF, exportPDFList, exportExcel, exportExcelList } from '@/api/equipmentMobilization/equipMaterialLedger'
...
export default {
      ...
      return [
        ...
        },
        {
          ...
          scopedSlots: {
            ...
            customRender: 'material_date'
          },
          onFilter: (value, record) => record.material_date.toLowerCase().includes(value.toLowerCase()),
          ...
          sorter: (a, b) => a.material_date.split('-').join('') - b.material_date.split('-').join('')
        },
        {
          title: '单据编号',
        ...
        },
        {
          ...
          align: 'right',
          scopedSlots: {
            customRender: 'amount_without_tax'
          }
        },
        {
          title: '收入税额（元）',
        ...
        },
        {
          ...
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'rec_org'
          },
          onFilter: (value, record) => record.rec_org.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
        ...
        },
        {
          ...
          align: 'right',
          scopedSlots: {
            customRender: 'expend_num'
          }
        },
        {
          title: '库存不含税金额（元）',
          align: 'right',
          dataIndex: 'expend_price_with_tax',
          scopedSlots: {
            customRender: 'expend_price_with_tax'
          }
        },
        {
          title: '库存价税合计（元）',
  ...
  methods: {
    ...
    onChangeDate (date, range) {
      this.queryParam.begin_material_date = range[0]
      this.queryParam.end_material_date = range[1]
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handlePurchase({ details: that.selectedRowKeys, flag: 0, menu_id: 90 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handlePurchase({ details: that.selectedRowKeys, flag: 0, menu_id: 90 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handlePurchase({ details: that.selectedRowKeys, menu_id: 90 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {
    ...
    handleSearch (selectedKeys, confirm, col) {
      if (col === 'material_date' || col === 'check_at') {
        this.refresh(true)
      } else {
        confirm()
      }

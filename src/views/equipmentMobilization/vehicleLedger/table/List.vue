...
import { handlePurchase, exportPDF, exportPDFList, exportExcel, exportExcelList } from '@/api/equipmentMobilization/vehicleLedger'
      ...
      return [
        ...
        },
        {
          ...
          scopedSlots: {
            ...
            customRender: 'buckle_time'
          },
          onFilter: (value, record) => record.buckle_time.toLowerCase().includes(value.toLowerCase()),
          ...
          sorter: (a, b) => a.buckle_time.split('-').join('') - b.buckle_time.split('-').join('')
        },
        {
          title: '单据编号',
        ...
        },
        {
          ...
          onFilter: (value, record) => record.car_no.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
        ...
        },
        {
          ...
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'planned_return_time'
          },
          onFilter: (value, record) => record.planned_return_time.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                // this.searchInput.focus()
          ...
          sorter: (a, b) => a.planned_return_time.split('-').join('') - b.planned_return_time.split('-').join('')
        },
        {
          title: '厂段修到期时间',
          dataIndex: 'expire',
          ...
          onFilter: (value, record) => record.expire.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                // this.searchInput.focus()
          ...
          sorter: (a, b) => a.expire.split('-').join('') - b.expire.split('-').join('')
        },
        {
          title: '走行部分',
          dataIndex: 'running',
          scopedSlots: {
            customRender: 'running'
          }
        },
        {
          title: '制动部分',
  ...
  methods: {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handlePurchase({ details: that.selectedRowKeys, flag: 0, menu_id: 92 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handlePurchase({ details: that.selectedRowKeys, flag: 0, menu_id: 92 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handlePurchase({ details: that.selectedRowKeys, menu_id: 92 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

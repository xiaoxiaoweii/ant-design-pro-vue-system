...
export default {
  ...
  watch: {
    $route (to, from) {
      ...
      if (from.path === '/equipmentMobilization/materialCheckList') {
        this.isrequired = true
        // 税率
        getDictionaryEnum({ pageNum: 1, pageSize: 13, dic_type_id: 1006 }).then(res => {
  ...
  methods: {
    ...
    async loadEditInfo (data) {
      ...
      if (data.id) {
        ...
        const formData = pick(data, ['serial_number', 'pro_unit', 'create_date', 'creator_user_name', 'item_check_date', 'contract_name', 'supplier_name', 'contract_no', 'remark'])
    ...
    saveOrSubmit (type) {
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          ...
          values.details = values.details.filter((x, i) => {
          ...
          for (let key in values) {
            const isTrue = values[key] === '' || values[key] == undefined || values[key] == null || values[key] === 'NaN' || values[key] === NaN
            if (isTrue) {
              delete values[key]
            }
          }

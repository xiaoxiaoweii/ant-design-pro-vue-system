<template>
  <div>
    ...
    >
      ...
      >{{ index + 1 + (current - 1) * 10 }}</span>

      <!-- 新搜索页面 -->
      <div slot="filterDropdown" slot-scope="{ setSelectedKeys, selectedKeys, confirm, clearFilters,column}" class="custom-filter-dropdown">
        <a-range-picker
          v-if="column.dataIndex==='use_org_name'||column.dataIndex==='xxx'"
...
import { handleLeaseprice, exportToExcel } from '@/api/pricepool/leaseprice'
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          onFilter: (value, record) => record.use_org_name,
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
        ...
        },
        {
          ...
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'rent_type'
          },
          onFilter: (value, record) => record.rent_type.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
        ...
        },
        {
          ...
          onFilter: (value, record) => record.tax.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
      ...
      loadData: parameter => {
        ...
        return handleLeaseprice(Object.assign(parameter, this.queryParam), 'retrieve').then(res => {
  ...
  methods: {
    onChange (date, range) {
      this.queryParam.begin_use_org_name = range[0]
      this.queryParam.end_use_org_name = range[1]
    },
    ...
    handleReset (clearFilters, col) {
      ...
      // this.queryParam.use_org_name = ''
      // this.queryParam.supplier_name= ''
      // this.queryParam.contract_no=''
      // this.queryParam.start_date=''
      // this.queryParam.xxx=''
      this.queryParam.name = ''
      col === 'use_org_name' && (this.range = [])
    },

    handleEdit (bool) {
      if (bool) if (this.selectedRows.length === 0 || this.selectedRows.length > 1) return this.noSelect('请选择一条记录进行编辑')
    ...
    //     okType: 'danger',
    //     onOk () {
    //       return handleLeaseprice({ ids: selectedRowKeys }, 'delete').then(res => res.status === '0' && that.refresh(true))
    //     },
    //     onCancel () { }
    //   })
    // },
    ...
    //     content: '确定提交已选择的记录?',
    //     onOk () {
    //       return handleLeaseprice({ ids: that.selectedRowKeys, flag: 0 }, 'submit').then(res => res.status === '0' && that.refresh(true))
    //     },
    //     onCancel () { }
    //   })
    // },
    ...
    //     content: '确定取回已选择的记录?',
    //     onOk () {
    //       return handleLeaseprice({ ids: that.selectedRowKeys }, 'cancel').then(res => res.status === '0' && that.refresh(true))
    //     },
    //     onCancel () { }
    //   })
    // },

<template>
  <div>
    <!-- 基本设备临时设施信息 -->
    <a-card class="card" title="设备维修保养记录-编辑" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
    ...
    >
      <s-table
        ...
        :pagination="{showSizeChanger: true, pageSizeOptions: ['15', '30', '45'], showQuickJumper: true, showTotal: total => `共 ${total} 条`}"
      >
        <div
          slot="filterDropdown"
...
import {contractsSearchById,selectRecord,handleMaintenanceRecord,queryOne} from '@/api/onsite/maintenanceRecord'
import { model, getAttachments, queryByCategory,selectLedger,querySingle} from '@/api/onsite/upload'
...
export default {
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryOne({id:data.id,menu_id:42}).then(res => {
          that.dataFrom=res.responseObject
          if(that.dataFrom['maintenance_way']){
            that.maintenanceWay=that.dataFrom['maintenance_way']
          }
          ...
          [
            ...
            'contract_no',      //合同号
            'dispatch_org',     //供应商名称
            'last_maintain_date',   //上次维修时间/里程
            'tax_rate',    //税率
            'tax_fee',    //税额(元)
            'tax_fee_maintenance_costs',    //不含税费用
            'tax_included_maintenance_costs',    //含税费用
            'people',    //参加维保主要人员
            'level',    //维护级别
            'content',    //维修保养工作内容
            'check_result',    //机械负责人验收意见
          ])
          formData.create_date = moment((that.dataFrom.create_date == null ? {} : that.dataFrom.create_date))
          form.setFieldsValue(formData)
          that.tax_rate=that.dataFrom.tax_rate
          that.tax_fee_maintenance_costs=that.dataFrom.tax_fee_maintenance_costs
          that.equip_code=that.dataFrom.equip_code          
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          ...
          if (type === 'update') {
            ...
            return handleMaintenanceRecord(values, 'update').then(res => {

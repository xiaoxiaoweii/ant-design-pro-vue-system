  ...
  <div>
    ...
    >
      <detail-list>
        ...
        <detail-list-item term="维修类型">{{ dataFrom.maintenance_type }}</detail-list-item>
        <detail-list-item term="管理号码">{{ dataFrom.manage_code }}</detail-list-item>
        <detail-list-item term="设备名称">{{ dataFrom.equip_name }}</detail-list-item>
        <detail-list-item term="规格型号">{{ dataFrom.spec }}</detail-list-item>
        <detail-list-item term="维修方式">{{ dataFrom.maintenance_way }}</detail-list-item>
        <detail-list-item term="合同号">{{ dataFrom.contract_no }}</detail-list-item>
        <detail-list-item term="供应商名称">{{ dataFrom.dispatch_org }}</detail-list-item>
        <detail-list-item term="上次维修时间/里程">{{ dataFrom.last_maintain_date }}</detail-list-item>
        <detail-list-item term="税率">{{ dataFrom.tax_rate }}</detail-list-item>
        <detail-list-item term="不含税单价">{{ dataFrom.tax_fee_maintenance_costs }}</detail-list-item>
        <detail-list-item term="税额(单)">{{ dataFrom.tax_fee }}</detail-list-item>
        <detail-list-item term="含税单价">{{ dataFrom.tax_included_maintenance_costs }}</detail-list-item>
        <detail-list-item term="参加维保主要人员">{{ dataFrom.people }}</detail-list-item>
        <detail-list-item term="维护级别">{{ dataFrom.level }}</detail-list-item>
        <detail-list-item class="remark" term="维修保养工作内容">{{ dataFrom.content }}</detail-list-item>
        <detail-list-item class="remark" term="机械负责人验收意见">{{ dataFrom.check_result }}</detail-list-item>
      </detail-list>
    </a-card>
...
import {queryOne,approve,reject} from '@/api/onsite/maintenanceRecord'
import { model, getAttachments, queryByCategory} from '@/api/onsite/upload'
...
export default {
  ...
  data () {
    return {
      ...
      activeTabKey: 'attach',
      tax_rate:6, //税率
      tax_included_maintenance_costs:0, //金额
      tax_fee:0, //税额
      fileList: [],
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryOne({id:data.id,menu_id:42}).then(res => { //根据id获取数据
        ...
        if (that.recording.process_inst_id&&that.recording.process_inst_id!="--") {
          handleQueryApprover({ menu_id: 42, process_inst_id: this.recording.process_inst_id }).then(res => {

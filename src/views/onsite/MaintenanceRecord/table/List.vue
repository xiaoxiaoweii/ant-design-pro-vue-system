<template>
  <div>
    ...
    >
      ...
      <a :class="classChange[index]" slot="contract_no" slot-scope="text, record, index" @click="clickEdit(text,index)" href="javascript:;">{{ text }}</a>
...
import { fuzzySearch,handleMaintenanceRecord} from '@/api/onsite/maintenanceRecord'
    ...
    columns () {
      ...
      return [
        ...
        },
        {
          ...
          onFilter: (value, record) => record.contract_no.toLowerCase().includes(value.toLowerCase()),
        ...
        },
        {
          ...
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'maintenance_type'
          },
          onFilter: (value, record) => record.maintenance_type.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                // this.searchInput.focus()
        ...
        {
          title: '维修费用',
          dataIndex: 'tax_fee_maintenance_costs',
          width: '220px',
          align: 'right',
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'tax_fee_maintenance_costs'
          },
          onFilter: (value, record) => record.tax_fee_maintenance_costs.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                // this.searchInput.focus()
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        // parameter
        return fuzzySearch(Object.assign(parameter, this.queryParam
                ,{creator_org_id:this.$store.state.menu_key||0,menu_id:42})).then(res => {          
  ...
  methods: {
    ...
    handleEdit (bool) { 
      if (bool) {
        ...
        this.$router.push({ name: 'maintenanceRecordEdit', params: {selectedRows:this.selectedRows[0],type:2} })
      } else {
        return this.$router.push({ name: 'maintenanceRecordNew',params: {type: 1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleMaintenanceRecord({ details: that.selectedRowKeys, menu_id: 42 }, 'delete').then(
            res => {
              if (res.msg == '删除成功') {
                that.noSelect(`删除成功`)
                that.refresh(true)
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          return handleMaintenanceRecord({ details: that.selectedRowKeys, flag:0, menu_id: 42 }, 'submit').then(res => {
            // res.status === '0'
    ...
    retrieve() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleMaintenanceRecord({ details: that.selectedRowKeys, menu_id: 42 }, 'retrieve').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

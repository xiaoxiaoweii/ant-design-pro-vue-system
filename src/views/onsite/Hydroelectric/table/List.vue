...
import { handleHydroelectric,fuzzySearch } from '@/api/onsite/hydroelectric'
      ...
      return [
        ...
        },
        {
          ...
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'fee_name'
          },
          onFilter: (value, record) => record.fee_name.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                // this.searchInput.focus()
        ...
        {
          title: '上期总读数',
          dataIndex: 'total_previous_num',
          align: 'right',
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'total_previous_num'
          },
          onFilter: (value, record) => record.total_previous_num.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                // this.searchInput.focus()
        ...
        {
          title: '本期总读数',
          dataIndex: 'total_current_num',
          align: 'right',
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'total_current_num'
          },
          onFilter: (value, record) => record.total_current_num.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                // this.searchInput.focus()
        ...
        },
        {
          ...
          onFilter: (value, record) => record.total_num.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                // this.searchInput.focus()
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return fuzzySearch(Object.assign(parameter, this.queryParam
                    ,{creator_org_id:this.$store.state.menu_key||0,menu_id:40})).then(res => {
          ...
          return res.result ||{ pageSize: 10000, 
                                pageNo:1, 
                                totalPage: 1, 
                                totalCount:1, 
  ...
  methods: {
    ...
    handleEdit (bool) { 
      if (bool) {
        ...
        this.$router.push({ name: 'hydroelectricEdit', params: {selectedRows:this.selectedRows[0],type:2} })
      } else {
        return this.$router.push({ name: 'hydroelectricNew',params: {type: 1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleHydroelectric({ details: that.selectedRowKeys, menu_id: 40 }, 'delete').then(
            res => {
              if (res.msg == '删除成功') {
                that.noSelect(`删除成功`)
                that.refresh(true)
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          return handleHydroelectric({ details: that.selectedRowKeys, flag: 0, menu_id: 40 }, 'submit').then(res => {
            // res.status === '0'
    ...
    retrieve() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleHydroelectric({ details: that.selectedRowKeys, menu_id: 40 }, 'retrieve').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

<template>
  <div>
    ...
    >
      ...
      <a :class="classChange[index]" slot="task_coding" slot-scope="text, record, index" @click="clickEdit(text,index)" href="javascript:;">{{ text }}</a>
      <div slot="filterDropdown" slot-scope="{ setSelectedKeys, selectedKeys, confirm, clearFilters, column }" class="custom-filter-dropdown">
        ...
        <a-range-picker 
          ...
          />
        <a-range-picker 
          v-else-if="column.dataIndex === 'start_date'" 
          v-model="startRange" 
          @change="onChangeStartDate" 
          style="width: 218px; margin-bottom: 8px; display: block;"
          />
        <a-range-picker 
          v-else-if="column.dataIndex === 'end_date'" 
          v-model="endRange" 
...
import { handleInventory,getInventoryList } from '@/api/onsite/inventory'
  ...
  computed: {
    dataColumns () {
      ...
      return [
        ...
        {
          title: '任务编码',
          dataIndex: 'task_coding',
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'task_coding'
          },
          onFilter: (value, record) => record.task_coding.toLowerCase().includes(value.toLowerCase()),
        ...
        },
        {
          ...
          sorter: (a, b) => {
            if (a.start_date === '--' || b.start_date === '--') {
              return a.start_date.length - b.start_date.length
            }
        ...
        },
        {
          ...
          onFilter: (value, record) => record.end_date.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                // this.searchInput.focus()
          ...
          sorter: (a, b) => {
            if (a.end_date === '--' || b.end_date === '--') {
              return a.end_date.length - b.end_date.length
            }
        ...
        },
        {
          ...
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'total_sum'
          },
          onFilter: (value, record) => record.total_sum.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                 this.searchInput.focus()
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        // parameter
        return getInventoryList(Object.assign(parameter, this.queryParam
                    ,{creator_org_id:this.$store.state.menu_key||0,menu_id:45})).then(res => {
  ...
  methods: {
    ...
    handleEdit (bool) { 
      if (bool) {
        ...
        this.$router.push({ name: 'inventoryEdit', params: {selectedRows:this.selectedRows[0],type:2} })
      } else {
        return this.$router.push({ name: 'inventoryNew',params: {type: 1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleInventory({ details: that.selectedRowKeys, menu_id: 45 }, 'delete').then(
            res => {
              if (res.msg == '删除成功') {
                that.noSelect(`删除成功`)
                that.refresh(true)
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          return handleInventory({ details: that.selectedRowKeys, flag: 0, menu_id: 45 }, 'submit').then(res => {
            // res.status === '0'
    ...
    retrieve() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleInventory({ details: that.selectedRowKeys, menu_id: 45 }, 'retrieve').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {
    ...
    onChangeStartDate (date, startRange) {
      this.queryParam.begin_start_date = startRange[0]
      this.queryParam.end_start_date = startRange[1]
    },
    ...
    handleSearch (selectedKeys, confirm, col) {
      if (['create_date','check_at','start_date','end_date'].includes(col)) {
        this.refresh(true)
      } else {
        confirm()
      }

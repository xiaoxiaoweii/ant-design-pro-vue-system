<template>
  <div>
    <!-- 基本设备临时设施信息 -->
    <a-card class="card" title="自有设备报停-编辑" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
...
import {handleNewspaperStop,queryOne,queryledger,selectCondInfo} from '@/api/onsite/newspaperStop'
...
export default {
  ...
  watch: {
    $route (to, from) {
      ...
      if(this.$route.params.type === 2){
        if (from.path === '/onsite/newspaperStopList') {
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryOne({id:data.id,menu_id:that.menu_id}).then(res => {
          ...
          formData.purchase_time = moment((that.dataFrom.purchase_time == null ? {} : that.dataFrom.purchase_time))
          formData.in_time = moment((that.dataFrom.in_time == null ? {} : that.dataFrom.in_time))
          formData.stop_time = moment((that.dataFrom.stop_time == null ? {} : that.dataFrom.stop_time))
          formData.start_time = moment((that.dataFrom.start_time == null ? {} : that.dataFrom.start_time))
    ...
    handleGoBack () {
      ...
      this.$router.push('newspaperStopList')
    },
    ...
    saveOrSubmit (type) {
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          ...
          //对备注进行校验
          if(values.stop_reason&&values.stop_reason.length>1000){
            return
          }
            ...
            if (type === 'submit') {
              this.$confirm({
                ...
                onOk () {
                  ...
                  return handleNewspaperStop(values, 'submit').then(res => {
          ...
          if (type === 'update') {
            ...
            return handleNewspaperStop(values, 'update').then(res => {

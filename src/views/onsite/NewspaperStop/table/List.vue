...
import { handleNewspaperStop,getNewspaperStopList } from '@/api/onsite/newspaperStop'
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        // parameter
        return getNewspaperStopList(Object.assign(parameter, this.queryParam
  ...
  methods: {
    ...
    handleEdit (bool) { 
      if (bool) {
        ...
        this.$router.push({ name: 'newspaperStopEdit', params: {selectedRows:this.selectedRows[0],type:2} })
      } else {
        return this.$router.push({ name: 'newspaperStopNew',params: {type: 1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleNewspaperStop({ details: that.selectedRowKeys, menu_id: that.menu_id }, 'delete').then(
            res => {
              if (res.msg == '删除成功') {
                that.noSelect(`删除成功`)
                that.refresh(true)
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          return handleNewspaperStop({ details: that.selectedRowKeys, flag: 0, menu_id: that.menu_id }, 'submit').then(res => {
            // res.status === '0'
    ...
    retrieve() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleNewspaperStop({ details: that.selectedRowKeys, menu_id: that.menu_id }, 'retrieve').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

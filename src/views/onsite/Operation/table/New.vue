<template>
  <div>
    <!-- 基本设备临时设施信息 -->
    <a-card class="card" title="设备运转记录表-新增" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
...
import {handleOperation,queryledger,selectCondInfo} from '@/api/onsite/operation'
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          ...
          if (type === 'save') {
            ...
            return handleOperation(values, 'save').then(res => {
...
/deep/ div.ant-col-18 > div.ant-row > div.ant-col-sm-6 {
  width: 80px;
}
.popover-wrapper {

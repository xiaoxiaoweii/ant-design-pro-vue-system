  ...
  <div>
    ...
    >
      <detail-list>
        ...
        <detail-list-item term="发料单位">{{ dataFrom.pro_unit }}</detail-list-item>
        <detail-list-item term="发料日期">{{ dataFrom.create_date|dayjs }}</detail-list-item>
        <detail-list-item term="收料单位">{{ dataFrom.rec_org }}</detail-list-item>
      </detail-list>
    </a-card>
...
import {queryOne,approve,reject} from '@/api/onsite/invoiceSheet'
...
export default {
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryOne({id:data.id,menu_id:43}).then(res => { //根据id获取数据
        ...
        if (that.recording.process_inst_id&&that.recording.process_inst_id!="--") {
          handleQueryApprover({ menu_id: 43, process_inst_id: this.recording.process_inst_id }).then(res => {

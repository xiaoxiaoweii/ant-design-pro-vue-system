<template>
  <div>
    <!-- 基本设备临时设施信息 -->
    <a-card class="card" title="设备材料发料单-编辑" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
...
import {handleInvoiceSheet,selectCondInfo,selectOrderSheet,queryOne} from '@/api/onsite/invoiceSheet'
...
export default {
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryOne({id:data.id,menu_id:43}).then(res => {
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          ...
          if (type === 'update') {
            ...
            return handleInvoiceSheet(values, 'update').then(res => {

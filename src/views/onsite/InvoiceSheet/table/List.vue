...
import { handleInvoiceSheet,getInvoiceSheetList } from '@/api/onsite/invoiceSheet'
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        // parameter
        return getInvoiceSheetList(Object.assign(parameter, this.queryParam
                    ,{creator_org_id:this.$store.state.menu_key||0,menu_id:43})).then(res => {
  ...
  methods: {
    ...
    handleEdit (bool) { 
      if (bool) {
        ...
        this.$router.push({ name: 'invoiceSheetEdit', params: {selectedRows:this.selectedRows[0],type:2} })
      } else {
        return this.$router.push({ name: 'invoiceSheetNew',params: {type: 1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleInvoiceSheet({ details: that.selectedRowKeys, menu_id: 43 }, 'delete').then(
            res => {
              if (res.msg == '删除成功') {
                that.noSelect(`删除成功`)
                that.refresh(true)
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          return handleInvoiceSheet({ details: that.selectedRowKeys, flag: 0, menu_id: 43 }, 'submit').then(res => {
            // res.status === '0'
    ...
    retrieve() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleInvoiceSheet({ details: that.selectedRowKeys, menu_id: 43 }, 'retrieve').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

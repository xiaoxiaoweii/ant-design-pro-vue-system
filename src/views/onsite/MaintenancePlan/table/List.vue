  ...
  <div>
    ...
    >
      ...
      <div slot="filterDropdown" slot-scope="{ setSelectedKeys, selectedKeys, confirm, clearFilters, column }" class="custom-filter-dropdown">
        ...
        <a-range-picker v-if="column.dataIndex === 'create_date'" v-model="range" @change="onChangeDate" style="width: 218px; margin-bottom: 8px; display: block;"/>
...
import { handleOperation,fuzzySearch } from '@/api/onsite/maintenancePlan'
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return fuzzySearch(Object.assign(parameter, this.queryParam
                    ,{creator_org_id:this.$store.state.menu_key||0,menu_id:46})).then(res => {
  ...
  methods: {
    ...
    handleEdit (bool) { 
      if (bool) {
        ...
        this.$router.push({ name: 'maintenancePlanEdit', params: {selectedRows:this.selectedRows[0],type:2} })
      } else {
        return this.$router.push({ name: 'maintenancePlanNew',params: {type: 1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleOperation({ details: that.selectedRowKeys, menu_id: 46 }, 'delete').then(
            res => {
              if (res.msg == '删除成功') {
                that.noSelect(`删除成功`)
                that.refresh(true)
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          return handleOperation({ details: that.selectedRowKeys, flag: 0, menu_id: 46 }, 'submit').then(res => {
            // res.status === '0'
    ...
    retrieve() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleOperation({ details: that.selectedRowKeys, menu_id: 46 }, 'retrieve').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

...
import { handleEquipOutPlan, exportToPdf as tableToPdf, exportToExcel, exporttoPDFList, exporttoExcelList, formStatusArray } from '@/api/equipExit/plan'
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return handleEquipOutPlan('retrieve', null, Object.assign(parameter, this.queryParam, { menu_id: this.menuCode, scope_code: this.scope_code })).then(res => {
  ...
  methods: {
    ...
    handleEdit (bool) {      
      if (bool) {
        ...
        this.$router.push({ name: 'equipExitPlanEdit', params: this.selectedRows[0], query: { menuCode: this.menuCode } })
      } else {
        return this.$router.push({ name: 'equipExitPlanNew', query: { menuCode: this.menuCode } })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk: () => {
          return handleEquipOutPlan('delete', { details: arrkeys, menu_id: this.menuCode }).then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk: () => {
          return handleEquipOutPlan('submit', { details: arrkeys, flag: 0, menu_id: this.menuCode }).then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk: () => {
          return handleEquipOutPlan('cancel', { details: arrkeys, menu_id: this.menuCode }).then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

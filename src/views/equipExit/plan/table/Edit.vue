<template>
  <div>
    ...
    <a-card class="card" :bordered="false" :tabList="tabList" :activeTabKey="activeTabKey">
...
import { handleEquipOutPlan, queryone, queryEquip } from '@/api/equipExit/plan'
...
export default {
  ...
  data () {
    return {
      ...
      selectData: { unit: [] },
      selectEquData: [],
      detailFields: ['manage_code', 'name', 'type', 'number', 'unit', 'tech_status', 'out_date'],
      selectFields: [],
      inputFields: [],
      datePickerFields: [],
      numberFields: [],
      ...
      detailFields: ['manage_code', 'name', 'type', 'number', 'unit', 'tech_status', 'out_date', 'remark'],
      attachColumns: [
        {
          title: '附件名称',
          dataIndex: 'name',
  ...
  computed: {
    ...
    mod () {
      return EXIT_EQUIP_PLAN_Modules.plan
    },
    date () {
      return moment()
    },
  ...
  watch: {
    $route (to, from) {
      if (to.path === '/equipExit/equipExitPlanEdit') {
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryone({ id: data.id, menu_id: this.menuCode }).then(res => {
          if (res.status === '0') {
            const formData = pick(res.responseObject, ['serial_number', 'creator_org_name', 'pro_unit', 'create_date','creator_user_name'])
            formData.create_date = moment(formData.create_date)
            form.setFieldsValue(formData)
            this.detailData = res.responseObject.details.map((detail,i )=> {
              detail.code = i+1
              detail.key = i+1
              return detail
            })
            this.detailData.map(d=>{
              this.selectedrows.push(d.manage_code)
            })
    ...
    handleGoBack () {
      ...
      this.$router.push('planExitList')
    },
    newDevice () {
      const length = this.detailData.length
      if (length > 0 && !this.detailData[this.detailData.length - 1].manage_code)
    ...
    handleSelectEquipment (equipment) {
      if(this.selectedrows.includes(equipment.manage_code)) {
        this.$info({
          title:'提示',
          content:'该记录不能重复选择'
        })
      ...
      this.selectedrows.push(equipment.manage_code)
      if (equipment) {
        ...
        this.handleChange(equipment.specification, this.currentRecord.key, 'type')
        this.handleChange(equipment.out_date, this.currentRecord.key, 'out_date')
        this.handleChange(equipment.technical_condition, this.currentRecord.key, 'tech_status')
        this.handleChange(equipment.remark, this.currentRecord.key, 'remark')
        ...
        if (!err) {
          this.$confirm({
            ...
            onOk: () => {
              ...
              // TODO: 编制单位,工程名称？
              return handleEquipOutPlan(type === 'save' ? 'update' : 'submit', Object.assign({ scope_code: this.scopeCode, menu_id: this.menuCode },
                values, {
                id: this.record.id,
                status: this.record.status,
                create_date: values.create_date.format('YYYY-MM-DD'),
                pro_unit_code: this.$store.state.pro_unit_code,
                creator_org_id: this.creatorOrgId
              })).then(res => {
  ...
  mounted () {
    ...
    this.$nextTick(() => {
      this.record = this.$route.params
      this.loadEditInfo(this.record)
      getDictionaryEnum({ dic_type_id: 1027 }).then(res => {
        if (res.status === '0')
          this.selectData.unit = res.responseList
      })
    })
  }
}

...
import { handleOwnEquipmentOut, queryEquip, queryone, queryEquipDetail } from '@/api/equipExit/own'
...
export default {
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryone({ id: data.id, menu_id: this.menuCode }).then(res => {
          if (res.status === '0') {
            this.owner_org_code = res.responseObject.owner_org_code || ''
            ...
            const formData = pick(res.responseObject, ['creator_org_name', 'pro_unit', 'location', 'creator_user_name', 'name', 'equip_code', 'manage_code', 'spec', 'manufacturer', 'purchase_date', 'prod_date', 'original_value', 'net_value', 'unit', 'depreciation_period', 'use_org', 'owner_org', 'serial_number'])
            formData.purchase_date = formData.purchase_date ? moment(formData.purchase_date) : null
            formData.prod_date = formData.prod_date ? moment(formData.prod_date) : null
    ...
    handleSelectEquipment (equipment) {
      if (equipment) {
        ...
        this.acceptData = equipment.acceptanceData || []
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          ...
          values = Object.assign({ scope_code: this.scopeCode, menu_id: this.menuCode },
            ...
            prod_date: values.purchase_date ? values.prod_date.format('YYYY-MM-DD') : null,
            pro_unit_code: this.$store.state.pro_unit_code,
            creator_org_id: this.scopeCode,
            equipment_category_code: this.record.equipment_category_code
          })
          ...
          if (type === 'save') {
            ...
            // TODO: 编制单位,工程名称？
            return handleOwnEquipmentOut(type === 'save' ? 'update' : 'submit',
              values).then(res => {
  ...
  activated () {
    ...
    this.record && this.loadEditInfo(this.record)
  },
  mounted () {
    this.$nextTick(() => {

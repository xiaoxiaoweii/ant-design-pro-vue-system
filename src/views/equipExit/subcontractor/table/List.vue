  ...
  <div>
    ...
    >
      ...
      >
        ...
        />
        <a-range-picker
          v-else-if="column.dataIndex==='out_time'"
          v-model="range1 "
          @change="onChange"
          @pressEnter="() => handleSearch(selectedKeys, confirm)"
          style="width: 188px; margin-bottom: 8px; display: block;"
...
import { handleSubcontractor, exportToPdf as tableToPdf, exportToExcel, exporttoPDFList, exporttoExcelList, formStatusArray } from '@/api/equipExit/subcontractor'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return handleSubcontractor('retrieve', null, Object.assign(parameter, this.queryParam, { menu_id: this.menuCode, scope_code: this.scope_code })).then(res => {
      ...
      menuBtns: [
        ...
        },
        {
          ...
          title: '刷新',
          type: 'primary',
          func: () => { this.queryParam = {}; this.range = [];this.range1 = [];  this.range2 = []; this.refresh(true) }
        }
      ],
    ...
    columns () {
      ...
      return [
        ...
        },
        {
          ...
          onFilter: (value, record) => record.subcontractor.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
        ...
        },
        {
          ...
          onFilter: (value, record) => record.out_time,
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
          ...
          sorter: (a, b) => a.out_time.split('-').join('') - b.out_time.split('-').join('')
        },
        {
          title: '退场设备数',
          dataIndex: 'number'
        ...
        },
        {
          ...
          sorter: (a, b) => a.check_at.split('-').join('') - b.check_at.split('-').join('')
        }
  ...
  methods: {
    ...
    onChange (date, range1) {
      this.queryParam.begin_out_time = range1[0]
      this.queryParam.end_out_time = range1[1]
    },
    ...
    handleSearch (selectedKeys, confirm, col) {
      if (col == 'create_date' || col == 'check_at'  || col == 'out_time') {
        this.refresh(true)
        return
      }
      confirm()
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'subcontractorEdit', params: this.selectedRows[0], query: { menuCode: this.menuCode } })
      } else {
        return this.$router.push({ name: 'subcontractorNew', query: { menuCode: this.menuCode } })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk: () => {
          return handleSubcontractor('delete', { details: arrkeys, menu_id: this.menuCode }).then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk: () => {
          return handleSubcontractor('submit', { details: arrkeys, flag: 0, menu_id: this.menuCode }).then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk: () => {
          return handleSubcontractor('cancel', { details: arrkeys, menu_id: this.menuCode }).then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

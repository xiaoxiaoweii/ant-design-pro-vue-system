...
export default {
  ...
  methods: {
    ...
    handleSelectcondInfo (cond) {
      if (cond) {
        this.subcontractor_code = cond.socialId
            ...
            if (type === 'submit') {
              this.$confirm({
                ...
                onOk: () => {
                  ...
                  // TODO: 编制单位,工程名称？
                  return handleSubcontractor(type === 'save' ? 'create' : 'submit', Object.assign({ menu_id: this.menuCode }, values, { out_time: values.out_time.format('YYYY-MM-DD'), create_date: values.create_date.format('YYYY-MM-DD'), pro_unit_code: this.$store.state.pro_unit_code, creator_org_id: this.scopeCode })).then(res => {

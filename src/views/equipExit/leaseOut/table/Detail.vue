  ...
  <div>
    ...
    <a-card class="card" title="租赁设备退场单" :bordered="false">
      <detail-list>
        ...
        <detail-list-item term="退场日期">{{ recording.out_time }}</detail-list-item>
        <detail-list-item term="编制单位">{{ recording.pro_unit }}</detail-list-item>
        <detail-list-item term="承租方">{{ recording.leasee }}</detail-list-item>
        <detail-list-item term="出租方">{{ recording.lessor }}</detail-list-item>
      </detail-list>
    </a-card>

    <!-- 设备明细 -->
    <a-card class="card" title="设备明细" :bordered="false">
      <a-table
        size="small"
        :columns="columns"
...
import { handleRentEquipOut, submitRequire, queryone, handleApprove, handleRefuse } from '@/api/equipExit/leaseOut'
import {
  downloadAttachment,
  getImgView,
  handleQueryApprover

...
import { handleRentEquipOut, exportToPdf as tableToPdf, exportToExcel, exporttoPDFList, exporttoExcelList, formStatusArray } from '@/api/equipExit/leaseOut'
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return handleRentEquipOut('retrieve', null, Object.assign(parameter, this.queryParam, { menu_id: this.menuCode, scope_code: this.scope_code })).then(res => {
    ...
    columns () {
      ...
      return [
        ...
        },
        {
          ...
          onFilter: (value, record) => record.leasee.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
        ...
        },
        {
          ...
          onFilter: (value, record) => record.lessor.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: (visible) => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'leaseOutEdit', params: this.selectedRows[0], query: { menuCode: this.menuCode } })
      } else {
        return this.$router.push({ name: 'leaseOutNew', query: { menuCode: this.menuCode } })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk: () => {
          return handleRentEquipOut('delete', { details: arrkeys, menu_id: this.menuCode }).then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk: () => {
          return handleRentEquipOut('submit', { details: arrkeys, flag: 0, menu_id: this.menuCode }).then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk: () => {
          return handleRentEquipOut('cancel', { details: arrkeys, menu_id: this.menuCode }).then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

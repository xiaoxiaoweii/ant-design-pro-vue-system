    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryone({ id: data.id, menu_id: this.menuCode }).then(res => {
          if (res.status === '0') {
            const formData = pick(res.responseObject, ['serial_number', 'leasee', 'creator_org_name', 'pro_unit', 'lessor', 'out_time'])
        ...
        if (!err) {
          this.$confirm({
            ...
            onOk: () => {
              ...
              // TODO: 编制单位,工程名称？
              return handleRentEquipOut(type === 'save' ? 'update' : 'submit', Object.assign({ scope_code: this.scopeCode, menu_id: this.menuCode },
                values, {
                id: this.record.id,
                status: this.record.status,
                out_time: values.out_time.format('YYYY-MM-DD'),
                pro_unit_code: this.$store.state.pro_unit_code,
                creator_org_id: this.scopeCode              })).then(res => {

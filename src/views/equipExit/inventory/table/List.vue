...
import { handleMaterialOutInventory, exportToPdf as tableToPdf, exportToExcel, exporttoPDFList, exporttoExcelList, formStatusArray } from '@/api/equipExit/inventory'
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return handleMaterialOutInventory('retrieve', null, Object.assign(parameter, this.queryParam, { menu_id: this.menuCode, scope_code: this.scope_code })).then(res => {
  ...
  computed: {
    ...
    columns () {
      ...
      return [
        ...
        {
          title: '转让价格',
          dataIndex: 'total_transfer_price',
          align:'right',
          scopedSlots: {
            customRender: 'total_transfer_price'
          }
        },
        {
          title: '状态',
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'materialsExitEdit', params: this.selectedRows[0], query: { menuCode: this.menuCode } })
      } else {
        return this.$router.push({ name: 'materialsExitNew', query: { menuCode: this.menuCode } })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk: () => {
          return handleMaterialOutInventory('delete', { details: arrkeys, menu_id: this.menuCode }).then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk: () => {
          return handleMaterialOutInventory('submit', { details: arrkeys, flag: 0, menu_id: this.menuCode }).then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk: () => {
          return handleMaterialOutInventory('cancel', { details: arrkeys, menu_id: this.menuCode }).then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

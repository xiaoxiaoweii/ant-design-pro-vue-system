    ...
    handleChange (value, key, column) {
      ...
      if (target) {
        ...
        if (column === 'num')
          ...
          if(target.plan_num){
          target.variable = isNaN(target.plan_num - target.num)?'':(target.plan_num - target.num).toFixed(2)}
        ...
        if (!err) {
          this.$confirm({
            ...
            onOk: () => {
              ...
              // TODO: 编制单位,工程名称？
              return handleMaterialOutInventory(type === 'save' ? 'create' : 'submit', Object.assign({ menu_id: this.menuCode },
                values, {

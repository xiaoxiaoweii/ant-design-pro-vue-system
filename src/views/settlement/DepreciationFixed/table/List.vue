...
import { handleDepreciationFixed, exportPDF, exportPDFList, exportExcel, exportExcelList } from '@/api/settlement/depreciationFixed'
...
export default {
    ...
    columns(){
      ...
      return [
        ...
        },
        {
          ...
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'total_depreciation'
          },
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        // parameter
        return handleDepreciationFixed(Object.assign(parameter, this.queryParam, this.orgparam), 'retrieve').then(res => {
      ...
      orgparam: {
        menu_id: 56,
        scope_code: this.$store.state.menu_key || this.$store.state.user.first_key
      },
  ...
  watch: {
    $route (to, from) {
      if (from.path === '/settlement/depreciationFixedNew' || from.path === '/settlement/depreciationFixedEdit') {
        this.refresh(true)
      }
    }
  },
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        if (this.selectedRows[0].dic_enum_name !== '已保存') {
          ...
          this.$refs.Stable.clearSelected()
          return
        }
        ...
        this.$router.push({ name: 'depreciationFixedEdit', params: this.selectedRows[0] })
      } else {
        return this.$router.push({ name: 'depreciationFixedNew', params: { type: 1 } })
      }
    },
    handleRead () {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handleDepreciationFixed({ details: that.selectedRowKeys, flag: 0, menu_id: 56 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleDepreciationFixed({ details: that.selectedRowKeys, flag: 0, menu_id: 56 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleDepreciationFixed({ details: that.selectedRowKeys, menu_id: 56 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

      ...
      if (data.id) {
        getDeviceList({ id: data.id, menu_id: 59 }).then(res => {
          ...
          if (this.recording.process_inst_id && this.recording.process_inst_id !== '--') {
            handleQueryApprover({ menu_id: 59, process_inst_id: this.recording.process_inst_id }).then(res => {
          ...
          const arr = ['price', 'amount', 'tax_amount', 'settlement_amount_excluding_tax', 'settlement_amount', 'deductions_money', 'other_fees', 'contract_add_amount_without_tax', 'contract_add_amount']
          arr.map(x => {

<template>
  <div>
    <a-card class="card" title="月度资金计划管理" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          ...
          <a-col :span="6">
            <a-form-item label="编制人" v-bind="formItemLayout" default-value="0">
                <a-input
                v-decorator="[
                'creator_user_name',
                {
                  rules: [{ message: '请输入供应商名称' }] 
                }
                ]"
                />
            </a-form-item>
...
import { getDeviceList,handleMonthlyCapital} from '@/api/settlement/MonthlyCapital'
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          align: 'center',
          scopedSlots: { customRender: 'current_payment_amount' }
        },
        {
          title: '开票金额开累(元)',
          dataIndex: 'invoice_amount',
          align: 'center',
          scopedSlots: { customRender: 'invoice_amount' }
        },
        {
          title: '结算金额开累(元)',
        ...
        },
        {
          ...
          align: 'center',
          scopedSlots: { customRender: 'cumulative_payment_amount' }
        },
        {
          title:'实际付款比例',
          dataIndex:'actual_payment_ratio',
          align:'center',
          scopedSlots:{ customRender:'actual_payment_ratio'}
        },
        {
          title: '付款比例限额',
          dataIndex: 'payment_ratio_term',
          align: 'center',
          scopedSlots: { customRender: 'payment_ratio_term' }
        },
        {
          title: '是否符合付款限额',
          dataIndex: 'payment_limit',
          align: 'center',
          scopedSlots: { customRender: 'payment_limit' }
        },
        {
          title: '备注',
          dataIndex: 'remark',
      ...
      tableData:[],
      loadData: parameter => {
        if (!this.queryParam.vehicle_check_id) {
          this.queryParam = {
            vehicle_check_id: '0'
          }
        }
  ...
  methods: {
    ...
    loadEditInfo(data) {
      ...
      if (data.id) {
        ...
        getDeviceList({ id: data.id }).then(res => {
          ...
          console.log('this is detaildata',this.detailData)
    ...
    newDevice() {
      ...
      this.detailData.push({
        ...
        actual_payment_ratio:'0',
        payment_ratio_term:'0',
        payment_limit:'',
        remark:''
      })
    },
        ...
        if (!err) {
          ...
          this.$confirm({
            ...
            onOk() {
              ...
              values.creator_user_name= values.creator_user_name==undefined?'':values.creator_user_name
             
              return handleMonthlyCapital(values, type === 'save' ? (that.recording ? 'update' : 'create') : 'submit').then(
                res => {

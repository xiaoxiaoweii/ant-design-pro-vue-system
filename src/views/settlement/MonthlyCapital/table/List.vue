...
import { handleMonthlyCapital, exportToExcel,exportToPDF} from '@/api/settlement/MonthlyCapital'
    ...
    columns(){
      ...
      return [
        ...
        },
        {
          ...
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'total_invoice_amount'
          }  
        },
        {
          title: '开累结算金额(元)',
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        ...
        return handleMonthlyCapital(Object.assign(parameter, this.queryParam), 'retrieve').then(res => {
  ...
  methods: {
    ...
    remove() {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk() {
          return handleMonthlyCapital({ ids: selectedRowKeys }, 'delete').then(res => res.status === '0' && that.refresh(true))
        },
        onCancel() {}
      })
    },
    ...
    submit() {
      ...
      this.$confirm({
        ...
        content: '确定提交已选择的记录?',
        onOk() {
          return handleMonthlyCapital({ ids: that.selectedRowKeys, flag: 0 }, 'submit').then(
    ...
    cancel() {
      ...
      this.$confirm({
        ...
        onOk() {
          ...
          return handleMonthlyCapital({ ids: that.selectedRowKeys,menu_id:'1'}, 'cancel').then(
            res => res.status === '0' && that.refresh(true)
          )
        },
        onCancel() {}

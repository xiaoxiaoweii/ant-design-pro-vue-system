<template>
  <div>
    <a-card class="card" title="设备材料费用结算单" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
            ...
            <a-form-item label="供应商" v-bind="formItemLayout">
              ...
              <a-select
                ...
                v-decorator="[
                  'supplier_name',
                  {
                    rules: [{ required: isrequired, message: '请选择供应商' }]
                  }
                ]"
              >
                <a-select-option
    ...
    >
      <div class="attachment" v-if="activeTabKey === 'attach'">
        <a-upload-dragger
          ...
          :fileList="fileList"
          @change="fileChange"
          :data="{model:'E007', type: 0, menu_id: 62}"
        >
...
import {
  ...
  settleDate,
  // getMaterialType
} from '@/api/settlement/MaterialCost'
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        {
          title: '设备材料名称',
          dataIndex: 'device_material_name',
          align: 'center',
          scopedSlots: { customRender: 'device_material_name' }
        },
        {
          title: '设备材料类别',
          dataIndex: 'device_material_type',
          align: 'center',
          scopedSlots: { customRender: 'device_material_type' }
        },
        {
          title: '规格型号',
  ...
  methods: {
        ...
        if (new Date(this.startDate).getTime() > new Date(this.endDate).getTime()) {
          ...
          setTimeout(() => {
            this.form.setFieldsValue({
              settlement_start_time: moment(this.endDate)
            })
          }, 0);
        }
      }
    ...
    endChange (date, dateString) {
      ...
      if (this.startDate) {
        if (new Date(this.endDate).getTime() < new Date(this.startDate).getTime()) {
          ...
          setTimeout(() => {
            this.form.setFieldsValue({
              settlement_end_time: moment(this.endDate)
            })
          }, 0);
        }
      }
      ...
      getLedger({ scope_code: this.$store.state.menu_key, pageSize: 13, pageNum: 1, supplier_name: value }).then(res => {
        ...
        this.detailData = res.responsePageInfo.list.map((x, i) => {
          ...
          getAmount({ contract_code: x.contract_code }).then(res => {
            ...
            x.contract_add_amount_without_tax = parseInt(x.sumObj.contract_add_amount_without_tax * 100 + x.money_excluding_tax * 100) / 100
            x.contract_add_amount = parseInt(x.sumObj.contract_add_amount * 100 + x.value_tax_total * 100) / 100
            this.handleChange(x.tax_rate, x.key, 'tax_rate')
          })
    ...
    handleGoBack () {
      ...
      this.$router.push('materialCost')
    },

    loadEditInfo (data) {
      const { form } = this
      console.log(`将加载 ${data.id} 信息到表单`)
      getSupplier({ scope_code: this.$store.state.menu_key }).then(res => {
        this.selectData = res.responseList.map((x, i) => {
          x.id = i + 1
          return x
        })
      })
      if (data.id) {
        ...
        getDeviceList({ id: data.id, menu_id: 62 }).then(res => {
        ...
        data.settlement_start_time && (data.settlement_start_time = moment(data.settlement_start_time))
        data.settlement_end_time && (data.settlement_end_time = moment(data.settlement_end_time))
    ...
    handleChange (value, key, column) {
      ...
      if (target) {
        ...
        if (['amount', 'unit_price_excluding_tax', 'tax_rate'].includes(column)) {
          ...
          // 判断单价 置空结算数据
          if (!target.unit_price_excluding_tax) {
            target.tax_amount = 0
            target.unit_price_excluding_tax = 0

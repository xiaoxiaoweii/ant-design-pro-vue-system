  ...
  <div>
    <a-card class="card" title="设备材料费用结算单" :bordered="false">
      <detail-list>
        ...
        <detail-list-item term="开始日期">{{ dataObj.settlement_start_time }}</detail-list-item>
        <detail-list-item term="结束日期">{{ dataObj.settlement_end_time }}</detail-list-item>
      ...
      if (data.id) {
        getDeviceList({ id: data.id, menu_id: 62 }).then(res => {
          ...
          if (this.recording.process_inst_id && this.recording.process_inst_id !== '--') {
            handleQueryApprover({ menu_id: 62, process_inst_id: this.recording.process_inst_id }).then(res => {

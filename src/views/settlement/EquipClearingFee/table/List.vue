...
import { handleApitalMaterial, exportToExcel } from '@/api/settlement/equipClearingFee'
...
export default {
    ...
    columns(){
      ...
      return [
        ...
        },
        {
          ...
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'payment_amount_total'
          }
        },
        {
          title: '开累未付金额',
          dataIndex: 'debt_amount_total',
          align: 'right',
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'debt_amount_total'
          }
        },
        {
          title: '本期结算金额合计（不含税）',
          dataIndex: 'current_settlement_total_excluding_tax',
          align: 'right',
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'current_settlement_total_excluding_tax'
          }
        },
        {
          title: '本期结算金额合计（含税）',
          dataIndex: 'current_settlement_tax_total',
          align: 'right',
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'current_settlement_tax_total'
          }
        },
        {
          title: '开累结算金额(不含税)',
          dataIndex: 'invoice_settlement_total_excluding_tax',
          align: 'right',
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'invoice_settlement_total_excluding_tax'
          }
        },
        {
          title: '开累结算金额(含税)',
          dataIndex: 'invoice_settlement_tax_total',
          align: 'right',
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'invoice_settlement_tax_total'
          }
        },
         {
          title: '状态',
        ...
        },
        {
        ...
        }, 
         {
          title: '审批时间',
          dataIndex: 'check_at',
          scopedSlots: { customRender: 'check_at' },
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        // parameter
        return handleApitalMaterial(Object.assign(parameter, this.queryParam, this.orgparam), 'retrieve').then(res => {
  ...
  watch: {
    $route (to, from) {
      if (from.path === '/settlement/equipClearingFeeNew' || from.path === '/settlement/equipClearingFeeEdit') {
        this.refresh(true)
      }
    }
  },
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'equipClearingFeeEdit', params: this.selectedRows[0] })
      } else {
        return this.$router.push({ name: 'equipClearingFeeNew', params: { type: 1 } })
      }
    },
    handleRead () {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 100 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 100 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, menu_id: 100 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

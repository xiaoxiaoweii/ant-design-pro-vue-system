  ...
  <div>
    ...
    >
      <detail-list>
        ...
        <detail-list-item term="供应商名称">{{ recording.supplier }}</detail-list-item>
        <detail-list-item term="编制人">{{ recording.creator_user_name }}</detail-list-item>
        <detail-list-item term="结算起始日期">{{ recording.settlement_start_time }}</detail-list-item>
        <detail-list-item term="结算终止日期">{{ recording.settlement_end_time }}</detail-list-item>
      </detail-list>
    </a-card>
...
import {getDeviceList,handleMaterialCost} from '@/api/settlement/MaterialCost'
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          key:'unit_price_excluding_tax'
        },
        {
          title:'不含税结算金额(元)',
          dataIndex:'money_excluding_tax',
          key:'money_excluding_tax'
        },
        {
          title:'本期开累结算金额(元)',
      ...
      detailFields: ['serial_number', 'contract_number', 'device_material_name', 'specifications', 'unit', 'amount','unit_price_excluding_tax','money_excluding_tax','settlement_amount',
      'tax_rate','tax_amount','value_tax_total','price_with_tax','total','remark'],
      attachColumns: [
        {
          title: '附件名称',
          dataIndex: 'name',
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        ...
        getAttachments({ module: modules.vehicleCost, id_in_module: data.id }).then(res => {
        ...
        if (!err) {
          ...
          this.$confirm({
            ...
            onOk () {
              ...
              return handleVehicleCost(values, type === 'save' ? (that.recording ? 'update' : 'create') : 'submit').then(res => {
    ...
    fileChange (info) {
      ...
      fileList = fileList.map((file) => {
        ...
        file.key = file.uid
        file.module = modules.vehicleCost

...
import { handleApitalMaterial, exportPDF, exportPDFList, exportExcel, exportExcelList } from '@/api/settlement/demolitionStatement'
...
export default {
    ...
    columns () {
      ...
      return [
        ...
        },
        {
          ...
          align: 'right',
          scopedSlots: {
            customRender: 'total_amount_excluding_tax'
          }
        },
        {
          title: '合计金额(含税)',
          dataIndex: 'total_amount_tax',
          align: 'right',
          scopedSlots: {
            customRender: 'total_amount_tax'
          }
        },
        {
          title: '项目开累合计金额(不含税)',
  ...
  watch: {
    $route (to, from) {
      if (from.path === '/settlement/demolitionStatementNew' || from.path === '/settlement/demolitionStatementEdit') {
        this.refresh(true)
      }
    }
  },
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'demolitionStatementEdit', params: this.selectedRows[0] })
      } else {
        return this.$router.push({ name: 'demolitionStatementNew', params: { type: 1 } })
      }
    },
    handleRead () {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 60 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 60 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, menu_id: 60 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

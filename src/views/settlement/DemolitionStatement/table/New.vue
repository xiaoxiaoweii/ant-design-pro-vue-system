            ...
            <a-form-item label="供应商" v-bind="formItemLayout">
              <a-input
                ...
                v-decorator="[
                  'supplier_name',
                  {
                    rules: [{ maxlength: 32, required: isrequired, message: '内容未输入或超过字符长度限制' }]
                  }
                ]"
              />
            </a-form-item>
    ...
    >
      <div class="attachment" v-if="activeTabKey === 'attach'">
        <a-upload-dragger
          ...
          :fileList="fileList"
          @change="fileChange"
          :data="{model:'E003', type: 0, menu_id: 60}"
        >
...
export default {
  ...
  data () {
    return {
      ...
      loadTable: parameter => {
        ...
        return queryAllContract(Object.assign(parameter, this.contractParam, { menu_id: 60 })).then(res => {
          // 索引
  ...
  watch: {
    $route (to, from) {
      ...
      if (from.path === '/settlement/demolitionStatement') {
  ...
  methods: {
    ...
    async handleName (val) {
      ...
      // 赋值
      let arrdata = {}
      await getAmount({ contract_code: 123 }).then(res => {
        Object.assign(arr, res.responseObject)
        arrdata = res.responseObject
      })
      let price_sum = 0
      let amount_sum = 0
      let tax_amount_sum = 0
      let settlement_sum = 0
      let settlement_widthout_sum = 0
      let all_sum = 0
      let all_widthout_sum = 0
      this.detailData = this.detailData.map(x => {
        if (x.key === this.newDataKey) {
          x.contract_code = arr.contract_code
          x.price = arr.price - 0
          x.amount = arr.amount - 0
          x.tax_rate = arr.tax_rate - 0
          x.sumObj = arrdata
          x.tax_amount = parseInt((x.price * x.tax_rate) * x.amount * 100) / 100
          x.settlement_amount_excluding_tax = (parseInt(x.price * x.amount * 100)) / 100
          x.settlement_amount_tax = parseInt((x.price * x.tax_rate + x.price) * x.amount * 100) / 100

          x.contract_add_amount = parseInt((arr.contract_add_amount * 100 + x.settlement_amount_tax * 100)) / 100
          x.contract_add_amount_without_tax = parseInt(arr.contract_add_amount_without_tax * 100 + x.settlement_amount_excluding_tax * 100) / 100
        }
        if (x.order_number !== '合计') {
          price_sum += parseInt(x.price * 100)
          amount_sum += parseInt(x.amount * 100)
          tax_amount_sum += parseInt(x.tax_amount * 100)
          settlement_sum += parseInt(x.settlement_amount_excluding_tax * 100)
          settlement_widthout_sum += parseInt(x.settlement_amount_tax * 100)
          all_sum += parseInt(x.contract_add_amount * 100)
          all_widthout_sum += parseInt(x.contract_add_amount_without_tax * 100)
        }
        return x
      })
      this.detailData.forEach(x => {
        if (x.order_number === '合计') {
          x.price = price_sum / 100
          x.amount = amount_sum / 100
          x.tax_amount = tax_amount_sum / 100
          x.settlement_amount_excluding_tax = settlement_sum / 100
          x.settlement_amount_tax = settlement_widthout_sum / 100
          x.contract_add_amount = all_sum / 100
          x.contract_add_amount_without_tax = all_widthout_sum / 100
        }
      })
    },
    ...
    handleGoBack () {
      ...
      this.$router.push('demolitionStatement')
    },
    ...
    handleChange (value, key, column) {
      ...
      if (target) {
        ...
        if (['price', 'amount', 'tax_rate', 'other_fees', 'deductions_money'].includes(column)) {
          target.price ? target.price - 0 : 0
          target.amount ? target.amount - 0 : 0
          target.tax_rate ? target.tax_rate - 0 : 0

          // 税额
          target.tax_amount = ((target.price * target.tax_rate) * target.amount).toFixed(2)
          ...
          // 不含税金额
          target.settlement_amount_excluding_tax = (target.price * target.amount + fee).toFixed(2)


          // 含税金额
          target.settlement_amount_tax = ((target.price * (1 + target.tax_rate)) * target.amount + fee * (1 + target.tax_rate)).toFixed(2)
          ...
          // 开累金额 含税
          target.contract_add_amount = parseInt(target.sumObj.contract_add_amount * 100 + target.settlement_amount_tax * 100) / 100
    ...
    saveOrSubmit (type) {
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          ...
          values.details.forEach((x, i) => {
            // if (x.order_number === '合计') values.details.splice(i, 1)
            for (let key in x) {
              if (!x[key]) {
                delete x[key]
              }

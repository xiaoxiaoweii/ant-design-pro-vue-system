...
import { handleApitalMaterial, exportToExcel } from '@/api/settlement/hydroPower'
  ...
  computed: {
    fields() {
      ...
      this.columns.forEach(col => {
        if (col.dataIndex === 'hydroPower' || !col.dataIndex) return
      ...
      return [
        ...
        },
        {
          ...
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
            customRender: 'pro_unit'
          },
          onFilter: (value, record) => record.pro_unit.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
        ...
        {
          title: '结算金额(不含税)',
          dataIndex: 'total_amount_without_tax',
          align: 'right',
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'total_amount_without_tax'
          },
        },
        {
          title: '结算金额(含税)',
        ...
        },
        {
          ...
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'settlement_amount_without_tax'
          },
        },
        {
          title: '项目开累结算金额(含税)',
          dataIndex: 'settlement_tax_amount',
          align: 'right',
          scopedSlots: {
            filterIcon: 'filterIcon',
            customRender: 'settlement_tax_amount'
          },
        },
         {
          title: '状态',
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'hydroPowerEdit', params: this.selectedRows[0] })
      } else {
        return this.$router.push({ name: 'hydroPowerNew', params: { type: 1 } })
      }
    },
    handleRead () {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 99 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 99 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, menu_id: 99 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

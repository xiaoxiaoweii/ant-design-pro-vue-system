      ...
      setTimeout(() => {
        validateFields((err, values) => {
          ...
          values.details = that.detailData.filter(x => {
            x.settlement_start_time1 && (x.settlement_start_time = x.settlement_start_time1.format('YYYY-MM-DD'))
            x.settlement_end_time1 && (x.settlement_end_time = x.settlement_end_time1.format('YYYY-MM-DD'))
            for (let key in x) {
              if (!x[key] && x[key] !== 0) {
                delete x[key]
              }

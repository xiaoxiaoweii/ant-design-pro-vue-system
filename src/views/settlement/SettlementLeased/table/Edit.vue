...
export default {
  ...
  methods: {
      ...
      if (data.id) {
        getDeviceList({ id: data.id, menu_id: 58 }).then(res => {
          this.detailData = res.responseObject.details.map((d, i) => {
            ...
            d.key = d.id
            d.order_number = i + 1
            d.settlement_start_time && (d.settlement_start_time1 = moment(d.settlement_start_time))
            d.settlement_end_time && (d.settlement_end_time1 = moment(d.settlement_end_time))
    ...
    remove (key) {
      const arr = ['rent_price_monthly', 'planned_quantity', 'other_fees',
        'unit_price_excluding_tax', 'deductions_money', 'settlement_amount_excluding_tax', 'settlement_amount', 'contract_add_amount_without_tax', 'contract_add_amount']

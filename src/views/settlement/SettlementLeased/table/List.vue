...
import { handleApitalMaterial, exportToExcel } from '@/api/settlement/settlementLeased'
...
export default {
  ...
  watch: {
    $route (to, from) {
      if (from.path === '/settlement/settlementLeasedNew' || from.path === '/settlement/settlementLeasedEdit') {
        this.refresh(true)
      }
    }
  },
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'settlementLeasedEdit', params: this.selectedRows[0] })
      } else {
        return this.$router.push({ name: 'settlementLeasedNew', params: { type: 1 } })
      }
    },
    handleRead () {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 58 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 58 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, menu_id: 58 }, 'cancel').then(res => {
            if (res.responseList.length) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {
                  if (that.selectedRows[i].id === res.responseList[j]) {

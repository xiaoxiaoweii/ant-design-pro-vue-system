<template>
  <div>
    <a-card class="card" title="租赁设备结算单" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
    ...
    >
      <div class="attachment" v-if="activeTabKey === 'attach'">
        <a-upload-dragger
          ...
          :fileList="fileList"
          @change="fileChange"
          :data="{model:'E005', type: 0, menu_id: 58}"
        >
...
export default {
  ...
  mounted () {
    console.log(this.selectData, '下拉框')
    this.$nextTick(() => {
      getSupplier({ scope_code: this.$store.state.menu_key || this.$store.state.first_key }).then(res => {
        this.selectData = res.responseList.map((x, i) => {
          x.id = i + 1
  ...
  methods: {
    ...
    supChange (value) {
      ...
      getLedger({ scope_code: this.$store.state.menu_key, pageSize: 13, pageNum: 1, supplier_name: value }).then(res => {
        ...
        const arr = [
          // 'rent_price_monthly', 
        // 'unit_price_excluding_tax',
         'planned_quantity', 'other_fees', 'deductions_money', 'settlement_amount_excluding_tax', 'settlement_amount', 'contract_add_amount_without_tax', 'contract_add_amount']
        arr.map(x => {
    ...
    handleGoBack () {
      ...
      this.$router.push('settlementLeasedList')
    },
    ...
    remove (key) {
      const arr = [
        // 'rent_price_monthly', 
      'planned_quantity', 'other_fees',

        // 'unit_price_excluding_tax', 
        'deductions_money', 'settlement_amount_excluding_tax', 'settlement_amount', 'contract_add_amount_without_tax', 'contract_add_amount']
      ...
      if (target) {
        ...
        if (['planned_quantity', 'rent_price_monthly', 'other_fees', 'deductions_money', 'tax_rate'].includes(column)) {
          ...
          'planned_quantity',
            ...
            // 'unit_price_excluding_tax',
             'other_fees', 'deductions_money', 'settlement_amount_excluding_tax', 'settlement_amount', 'contract_add_amount_without_tax', 'contract_add_amount']
          arr.map(x => {
            newData.forEach(d => {
              if (d.order_number === '合计') {
                d[x] = this.data_sum(x)

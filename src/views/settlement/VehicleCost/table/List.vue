...
import { handleApitalMaterial, exportToExcel } from '@/api/settlement/VehicleCost'
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'vehicleCostEdit', params: this.selectedRows[0] })
      } else {
        return this.$router.push({ name: 'vehicleCostNew', params: { type: 1 } })
      }
    },
    handleRead () {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 61 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 61 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, menu_id: 61 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

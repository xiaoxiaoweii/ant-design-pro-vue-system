      ...
      if (data.id) {
        ...
        getDeviceList({ id: data.id, menu_id: 61 }).then(res => {
          this.detailData = res.responseObject.detail.map((d, i) => {
            d.editable = true
            d.isNew = true
            d.settlement_start_time1 = (d.settlement_start_time == null ? null : moment(d.settlement_start_time))
            d.settlement_end_time1 = (d.settlement_end_time == null ? null : moment(d.settlement_end_time))
            d.key = d.id
            d.order_number = i + 1
            d.contract_code && getAmount({ contract_code: d.contract_code }).then(res => {
              this.sumObj = res.responseObject
              d.contract_add_amount_without_tax = this.sumObj.contract_add_amount_without_tax
              d.contract_add_amount = this.sumObj.contract_add_amount
            })

...
import { handleApitalMaterial, exportToExcel } from '@/api/settlement/apitalMaterial'
...
export default {
  ...
  watch: {
    $route (to, from) {
      if (from.path === '/settlement/apitalMaterialNew' || from.path === '/settlement/apitalMaterialEdit') {
        this.refresh(true)
      }
    }
  },
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        ...
        this.$router.push({ name: 'apitalMaterialEdit', params: this.selectedRows[0] })
      } else {
        return this.$router.push({ name: 'apitalMaterialNew', params: { type: 1 } })
      }
    },
    handleRead () {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 57 }, 'delete').then(
            res => {
    ...
    submit () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, flag: 0, menu_id: 57 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel () {
      ...
      this.$confirm({
        ...
        onOk () {
          document.querySelector('.ant-modal-confirm-btns>.ant-btn-default').style.visibility = "hidden"
          return handleApitalMaterial({ details: that.selectedRowKeys, menu_id: 57 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

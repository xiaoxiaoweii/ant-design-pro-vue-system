...
import { handlePurchase, submitRequire,queryone } from '@/api/safety/plan'
...
export default {
  ...
  data () {
    return {
      ...
      selectData: {},
      selectFields: [],
      inputFields: ['name', 'post', 'equip', 'check_item', 'ison', 'remark'],
      timeFields: [''],  
      datePickerFields:['plan_time'],
      numberFields: [],
      memberLoading: false,
      loading: false,
      saveLoading: false,
      ...
      columns: [
       ...
       
        {
          ...
          key: 'plan_time',
          align: 'center',
          scopedSlots: { customRender: 'plan_time' }
        },
        {
          title: '添乘设备',
          dataIndex: 'equip',
      ...
      detailFields: ['code', 'name', 'post', 'plan_time', 'equip', 'check_item', 'ison', 'remark'],
      attachColumns: [
        {
          title: '附件名称',
          dataIndex: 'file_name',
      ...
      if (data.id) {
         ...
         queryone({id:data.id}).then(res => {
          ...
          this.detailData = res.responseObject.detail.map((d,i) => {
            ...
            d.plan_time = moment(d.plan_time)
            d.code=i+1
            d.key =d.id+''
            d.editable = true
            d.isNew = true
        ...
        getAttachments({ module: modules.plan, id_in_module: data.id }).then(res => {
        ...
        const formData = pick(data, [ 'create_date', 'creator_user_name', 'creator_org_name', 'name','post'])

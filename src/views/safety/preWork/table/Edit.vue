...
import { getDeviceList, handlePurchase, submitRequire,queryone} from '@/api/safety/preWork'
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        ...
        getAttachments({id_in_module: data.id, module:modules.preWork}).then(res => {
          console.log(res)
        ...
        const formData = pick(data, ['edu_place','creator_org_name','creator_user_name','target_audience','planned_hour', 'hour','create_date','lecturer', 'person_num','content','feedback','edu_movie'])

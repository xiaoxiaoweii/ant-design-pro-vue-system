...
import { handleTransform, exportToExcel, exportToPDF,queryone } from '@/api/planManagement/transform'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  data() {
    return {
      ...
      detailFields: ['fee_item','unit','num','price_without_tax','tax_rate','total_without_tax','total_tax','sum_with_tax','remark'
      ],
      attachColumns: [
        {
          title: '附件名称',
  ...
  watch: {
    $route(to, from) {
      if (to.path === '/planManagement/facitransformsEdit') {
  ...
  methods: {
    ...
    newDevice() {
      ...
      if (length > 0 && !this.detailData[this.detailData.length - 1].fee_item)
    ...
    handleChange(value, key, column) {
      ...
      if (target) {
        target[column] = value
        target.total_tax = isNaN((target.num * target.price_without_tax * target.tax_rate/100).toFixed(2))?'':(target.num * target.price_without_tax * target.tax_rate/100).toFixed(2)
        ...
        validateFields((err, values) => {
          if (!err) {
            ...
            if (type === 'submit') {
              ...
              return handleTransform(values, 'update').then(res => {

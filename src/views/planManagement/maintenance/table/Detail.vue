<template>
  <div>
    ...
    <a-card :bordered="false" v-if="!recording.type" title="审批意见">
      <a-textarea placeholder="请输入审批意见, 1000字以内" :maxlength="1000" @change="txtChange" :value="noted" />
    </a-card>
...
import { handleApprove, handleRefuse,exportToExcel,exportToPDF ,queryone} from '@/api/planManagement/maintenance'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
  ...
  methods: {
    ...
    async loadEditInfo(data) {
      ...
      if (this.recording.process_inst_id) {
          handleQueryApprover({ menu_id: 27, process_inst_id: this.recording.process_inst_id }).then(res => {
...
/deep/ div.ant-row>div>div.ant-row {
  display: flex;
  flex-direction: row;
  justify-content: space-between;
}
/deep/ div.ant-row>div>div.ant-row>div.ant-col-sm-2{
  width:110px
}

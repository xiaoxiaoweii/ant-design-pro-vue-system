...
import { handleMaintenance, exportToExcel, exportToPDF } from '@/api/planManagement/maintenance'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return handleMaintenance(Object.assign(parameter, this.queryParam,this.orgparam), 'retrieve').then(res => {
  ...
  watch: {
    $route(to, from) {
      if (from.path === '/planManagement/maintenanceNew' || from.path === '/planManagement/maintenanceEdit') {
        this.refresh(true)
      }
    }
  },
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        if (this.selectedRows.length === 0 || this.selectedRows.length > 1) return this.noSelect('请选择一条记录进行编辑')
         ...
         this.$router.push({name:'maintenanceEdit',params:this.selectedRows[0]})   
      } else {
        return this.$router.push({ name: 'maintenanceNew',params:{type:1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleMaintenance({ details: arrkeys,menu_id:27}, 'delete').then(
            res => {
    ...
    submit() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleMaintenance({ details: arrkeys, flag: 0, menu_id: 27 }, 'submit').then(res => {
            if (res.status==='1') {
              that.noSelect('提交失败')
              that.refresh(true)
              return
    ...
    cancel() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleMaintenance({ details: arrkeys, menu_id: 27 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

  ...
  <div>
    ...
    >
      <detail-list>
        ...
        <detail-list-item term="设备名称">{{ recording.equipment_name }}</detail-list-item>
        <detail-list-item term="规格型号">{{ recording.specification }}</detail-list-item>
        <detail-list-item term="费用预算费用">{{ recording.repair_budget }}</detail-list-item>
        <detail-list-item term="设备技术状况">{{ recording.tech_status }}</detail-list-item>
        <detail-list-item term="维修改造项目">{{ recording.fix_item }}</detail-list-item>
...
import { getDeviceList, handlePurchase, submitRequire,queryone} from '@/api/planManagement/maintenance'
...
export default {
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryone ({ id: data.id, menu_id: 3 }).then(res => {
          this.number = true
        })

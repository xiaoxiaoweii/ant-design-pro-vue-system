<template>
  <div>
    <!-- 基本购置单信息 -->
    <a-card class="card" title="临时设施安拆申请单" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
    ...
    >
      <div class="attachment" v-if="activeTabKey === 'picture'">
        <a-upload-dragger
          ...
          :data="{ model: mod, type: 0 ,menu_id:29}"
        >
...
import { handleFacility, exportToExcel, exportToPDF,queryone} from '@/api/planManagement/facilities'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  data() {
    return {
      ...
      detailData: [],
      columnsTitle: [
        '设备名称','设备编码','规格型号','单位','数量','不含税单价(元)','税率(%)','含税单价(元)','税额','不含税金额','含税金额','供应商名称','备注'
      ],
      columns: [
        {
          title: '序号',
      ...
      detailFields: ['name','specification','equipment_code','unit','num','price_without_tax', "tax_rate",'price_with_tax','tax_fee','total_without_tax','total_with_tax','supplier_name',
        'remark'
      ],
      attachColumns: [
        {
  ...
  watch: {
    $route(to, from) {
      if (to.path === '/planManagement/facilitiesEdit') {
  ...
  methods: {
    ...
    handleGoBack() {
      ...
      this.$router.push({ path: '/planManagement/facilitiesList' })
    },
            ...
            if (type === 'submit') {
              ...
              this.$confirm({
                ...
                onOk() {
                  ...
                  return handleFacility(values, 'submit').then(res => {
              ...
              return handleFacility(values, 'update').then(res => {
...
/deep/ .line-table tr td:nth-child(13) {
  font-size: 0 !important;
}

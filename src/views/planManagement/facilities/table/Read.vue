...
import { getDeviceList, handlePurchase, submitRequire,queryone,queryTaxRate} from '@/api/planManagement/facilities'
...
export default {
  ...
  data () {
    return {
      ...
      detailFields: ['manage_code','name','specification', 'num', 'unit','price_without_tax','tax_rate','price_with_tax','sum_with_tax','supplier_name' ,'remark'],
      sumData:[{
        all_total: '合计',
        specification1: '',
        num1: '',
  ...
  methods: {
    ...
    loadEditInfo (data) {
      ...
      if (data.id) {
        queryone ({ id: data.id, menu_id: 5 }).then(res => {

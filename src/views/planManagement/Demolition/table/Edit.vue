<template>
  <div>
    <!-- 基本购置单信息 -->
    <a-card class="card" title="设备安拆申请单" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
    ...
    >
      <div class="attachment" v-if="activeTabKey === 'picture'">
        <a-upload-dragger
          ...
          :data="{ model: mod, type: 0 ,menu_id:28}"
        >
...
import { handleDemolition, exportToExcel, exportToPDF,queryone,queryByLedger } from '@/api/planManagement/demolition'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  data() {
    return {
      ...
      detailData: [],
      columnsTitle: [
        '设备名称','设备编码','规格型号','单位','不含税单价(元)','税率(%)','含税单价(元)','税额','不含税总额','含税总额','供应商名称','备注'
      ],
      columns: [
        {
          title: '序号',
      ...
      detailFields: ['name','specification','equipment_code','unit','price_without_tax', "tax_rate",'price_with_tax','tax_fee','total_without_tax','total_with_tax','supplier_name',
        'remark'
      ],
      attachColumns: [
        {
      ...
      loadData: parameter => {
        ...
        return queryByLedger(Object.assign(parameter, {scope_code:this.$store.state.menu_key}), 'retrieve').then(res => {
          // 索引
  ...
  watch: {
    $route(to, from) {
      if (to.path === '/planManagement/DemolitionEdit') {
  ...
  methods: {
    ...
    async loadEditInfo(data) {
      ...
      if (data.id) {
        await queryone({ id: data.id, menu_id: 28 }).then(res => {
    ...
    handleGoBack() {
      ...
      this.$router.push({ path: '/planManagement/DemolitionList' })
    },
    ...
    handleChange(value, key, column) {
      ...
      if (target) {
        target[column] = value
        target.tax_fee = isNaN((target.price_without_tax * target.tax_rate/100).toFixed(2))?'':(target.price_without_tax * target.tax_rate/100).toFixed(2)
        ...
        target.total_without_tax = isNaN((target.price_without_tax).toFixed(2))?'':(target.price_without_tax).toFixed(2)
        target.total_with_tax = isNaN((target.price_with_tax*1).toFixed(2))?'':(target.price_with_tax*1).toFixed(2)
        this.detailData = newData
      }
    },
        ...
        validateFields((err, values) => {
          if (!err) {
            ...
            values.details =that.detailData.map(x=>{
              if(x.total_with_tax) x.total_with_tax = (x.total_with_tax).toString()
              return  x
            })
            for(let key in values.details) {
                if(values.details[key].total_with_tax>9999999999999999){
            ...
            if (type === 'submit') {
              ...
              this.$confirm({
                ...
                onOk() {
                  ...
                  return handleDemolition(values, 'submit').then(res => {
              ...
              return handleDemolition(values, 'update').then(res => {
...
/deep/ .line-table tr td:nth-child(12) {
  font-size: 0 !important;
}

...
import { handleDemolition, exportToExcel, exportToPDF,queryByLedger } from '@/api/planManagement/demolition'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  data() {
    return {
      ...
      detailData: [],
      columnsTitle: [
        '设备名称','设备编码','规格型号','单位','不含税单价(元)','税率(%)',
        '含税单价(元)','税额','不含税总额','含税总额','供应商名称','备注'
      ],
      columns: [
        {
          title: '序号',
  ...
  methods: {
    ...
    handleChange(value, key, column) {
      ...
      if (target) {
        ...
        target.total_with_tax = isNaN(target.price_with_tax)?'':(target.price_with_tax*1).toFixed(2)
        this.detailData = newData
      }
    },
        ...
        validateFields((err, values) => {
          if (!err) {
            ...
            if (type === 'submit') {
              ...
              return handleDemolition(values, 'create').then(res => {

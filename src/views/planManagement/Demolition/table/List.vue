...
import { handleDemolition, exportToExcel, exportToPDF } from '@/api/planManagement/demolition'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return handleDemolition(Object.assign(parameter, this.queryParam,this.orgparam), 'retrieve').then(res => {
  ...
  watch: {
    $route(to, from) {
      if (from.path === '/planManagement/DemolitionNew' || from.path === '/planManagement/DemolitionEdit') {
        this.refresh(true)
      }
    }
  },
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        if (this.selectedRows.length === 0 || this.selectedRows.length > 1) return this.noSelect('请选择一条记录进行编辑')
         ...
         this.$router.push({name:'DemolitionEdit',params:this.selectedRows[0]})   
      } else {
        return this.$router.push({ name: 'DemolitionNew',params:{type:1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleDemolition({ details: arrkeys,menu_id:28}, 'delete').then(
            res => {
    ...
    submit() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleDemolition({ details: arrkeys, flag: 0, menu_id: 28 }, 'submit').then(res => {
            if (res.status==='1') {
              that.noSelect('提交失败')
              that.refresh(true)
              return
    ...
    cancel() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleDemolition({ details: arrkeys, menu_id: 28 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

...
import { handlePurchase, submitRequire,queryone,queryTaxRate,getSerialNumber} from '@/api/planManagement/demolition'
...
export default {
  ...
  data () {
    return {
      ...
      detailFields: ['name','specification', 'num','supplier_name', 'unit','price_without_tax','tax_rate','price_with_tax','sum_with_tax', 'planned_arrival_date','remark'],
      sumData:[{
        all_total: '合计',                                                                                       
        specification1: '',
        num1: '',
      ...
      columnsSum: [
        ...
        },
        {
          ...
          width: '150px',
          align:'center',
          scopedSlots: { customRender: 'tax_fee1' }
  ...
  computed: {
    ...
    getSum () {
      ...
      console.log(count)
      this.sumData[0].sum_with_tax1 = count.toFixed(2)
      return this.sumData
    }
  },
  mounted () {
      ...
      if (data.id) {
        queryone ({ id: data.id, menu_id: 4 }).then(res => {
          this.detailData = res.responseObject.detail.map(d => {
            ...
            d.price_with_tax=d.price_with_tax.toFixed(2)

...
import { handlePurchase, submitRequire, queryone ,queryProjectName,queryByEquipment} from '@/api/planManagement/require'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  watch: {
    $route(to, from) {
      if (to.path === '/planManagement/requireEdit') {
        this.params=this.$route.params        
        this.$nextTick(() => {
          this.loadEditInfo(this.params)
          this.isrequired = true
  ...
  methods: {
      ...
      if (data.id) {
        await queryone({ id: data.id, menu_id: 15 }).then(res => {
          this.detailData = res.responseObject.details.map((d,i) => {
            ...
            d.estimated_in_date && (d.estimated_in_date1 = moment(d.estimated_in_date))
            d.estimated_out_date && (d.estimated_out_date1 = moment(d.estimated_out_date))
            d.key = d.id
           if (d.number==0) d.number=''
    ...
    newDevice() {
      ...
      // 限制只能同时填写一条数据
      if (length > 0 && !this.detailData[this.detailData.length - 1].equipment_name) return this.noSelect('请填完上一条数据')

...
import { handlePurchase, exportToExcel, exportToPDF, formStatusLabel, formStatusArray } from '@/api/planManagement/require'
const FileSaver = require('file-saver')
// require('@/utils/colResizable-1.6.min')
export default {
  name: 'TableList',
  components: {
    STable,
  ...
  methods: {
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handlePurchase({ details: arrkeys,menu_id:15}, 'delete').then(
            res => {
    ...
    submit() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handlePurchase({ details: arrkeys, flag: 0, menu_id: 15 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handlePurchase({ details: arrkeys, menu_id: 15 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

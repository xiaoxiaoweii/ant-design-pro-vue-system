<template>
  <div>
    ...
    <footer-tool-bar :style="{ width: isSideMenu() && isDesktop() ? `calc(100% - ${sidebarOpened ? 256 : 80}px)` : '100%'}">
      ...
      <a-button v-if="this.recording.type!=='read'"
        type="primary"
        @click="refuse"
        :loading="saveLoading"
      >打回</a-button>
...
import { handleApprove, handleRefuse,exportToExcel,exportToPDF ,getDeviceList} from '@/api/planManagement/temporary'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
      ...
      if (data.id) {
        await getDeviceList({ id: data.id, menu_id: 16 }).then(res => {
          ...
          this.detailData = res.responseObject.details.map(d => {
            ...
            if (d.total_power===0) d.total_power = ''
        ...
        if (this.recording.process_inst_id) {
          handleQueryApprover({ menu_id: 16, process_inst_id: this.recording.process_inst_id }).then(res => {

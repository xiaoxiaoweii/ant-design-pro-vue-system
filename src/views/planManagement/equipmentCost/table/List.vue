...
import { handleEquipmentLease, exportToExcel } from '@/api/planManagement/equipmentLease'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return handleEquipmentLease(Object.assign(parameter, this.queryParam,this.orgparam), 'retrieve').then(res => {
          res.responsePageInfo.list = res.responsePageInfo.list.map((x, index)=> {
            for (let i in x) {
              if (x[i] === null && i !== 'process_inst_id') x[i] = '--'
            }
            if (x.total==0) x.total = '--'
            if (x.is_approve === 1) {
  ...
  watch: {
    $route(to, from) {
      if (from.path === '/planManagement/equipmentLeaseNew' || from.path === '/planManagement/equipmentLeaseEdit') {
        this.refresh(true)
      }
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        if (this.selectedRows.length === 0 || this.selectedRows.length > 1) return this.noSelect('请选择一条记录进行编辑')
         ...
         this.$router.push({name:'equipmentLeaseEdit',params:this.selectedRows[0]})   
      } else {
        return this.$router.push({ name: 'equipmentLeaseNew',params:{type:1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleEquipmentLease({ details: arrkeys,menu_id:22}, 'delete').then(
            res => {
    ...
    submit() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleEquipmentLease({ details: arrkeys, flag: 0, menu_id: 22 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleEquipmentLease({ details: arrkeys, menu_id: 18 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

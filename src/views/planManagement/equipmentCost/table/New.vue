...
import { handleEquipmentLease, exportToExcel, exportToPDF } from '@/api/planManagement/equipmentLease'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  data() {
    return {
      ...
      inputFields: ['supplier_name', 'remark'],
      datePickerFields: ['planned_in_date1', 'planned_out_date1'],
      numberFields: ['price_without_tax', 'planned_quantity'],
      numberFields2: ['num'],
  ...
  methods: {
    ...
    handleOk() {
      ...
      // this.$refs.myinput[0]._props.disabled=true
    ...
    handleChange(value, key, column) {
      ...
      if (target) {
        ...
        target.sum_with_tax = isNaN((target.num * target.price_with_tax*target.planned_quantity).toFixed(2))
          ? ''
          : (target.num * target.price_with_tax*target.planned_quantity).toFixed(2)

        if (isNaN(target.sum_tax_fee)) target.sum_tax_fee = ''
        target.sum_tax_fee = isNaN((target.sum_with_tax - target.tax_fee).toFixed(2))
          ? ''
          : (target.sum_with_tax - target.tax_fee).toFixed(2)
        ...
        validateFields((err, values) => {
          if (!err) {
            ...
            if (type === 'submit') {
              ...
              return handleEquipmentLease(values, 'create').then(res => {

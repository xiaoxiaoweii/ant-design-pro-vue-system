<template>
  <div>
    <!-- 基本购置单信息 -->
    <a-card class="card" title="材料配件采购申请单" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
    ...
    >
      <div class="attachment" v-if="activeTabKey === 'picture'">
        <a-upload-dragger
          ...
          :data="{ model: mod, type: 0 ,menu_id:26}"
        >
...
import { handleMaterial, exportToExcel, exportToPDF, queryone,queryMaterial } from '@/api/planManagement/material'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          width: '150px',
          align: 'center',
          scopedSlots: { customRender: 'planned_arrival_date1' }
        },
        {
          title: '操作',
          key: 'action',
      ...
      loadData: parameter => {
        ...
        return queryMaterial(Object.assign(parameter, {scope_code:this.$store.state.menu_key},this.queryParam), 'retrieve').then(res => {
          // 索引
  ...
  watch: {
    $route(to, from) {
      if (to.path === '/planManagement/materialEdit') {
  ...
  methods: {
    ...
    async loadEditInfo(data) {
      ...
      if (data.id) {
        await queryone({ id: data.id, menu_id: 26 }).then(res => {
    ...
    handleOk() {
      ...
      this.detailData.map(d=>{
        if(d.name==arr[0].material_name&&d.specification==arr[0].specification&&d.material_code==arr[0].material_code) {
          this.$info({
            title:'提示',
            content:'该记录不能重复选择'
          })
    ...
    newDevice() {
      ...
      if (length > 0 && !this.detailData[this.detailData.length - 1].material_code)
        ...
        validateFields((err, values) => {
          if (!err) {
            ...
            values.details = that.detailData.map(x => {
              x.total_without_tax&&(x.total_without_tax = (x.total_without_tax).toString())
              x.planned_arrival_date1&&(x.planned_arrival_date = x.planned_arrival_date1.format('YYYY-MM-DD'))
              return x
            })
            ...
            if (type === 'submit') {
              ...
              values.details.map((d, i) => {
                for (var key in d) {
                  if (d[key] == null || d[key] == '') {
                    if (key != 'remark'&&key !='planned_arrival_date') {
              ...
              this.$confirm({
                ...
                onOk() {
                  ...
                  return handleMaterial(values, 'submit').then(res => {
              ...
              return handleMaterial(values, 'update').then(res => {

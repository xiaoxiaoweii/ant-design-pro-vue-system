<template>
  <div>
    <!-- 基本购置单信息 -->
    <a-card class="card" title="设备材料配件采购申请单" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8">
...
import { handleMaterial, exportToExcel, exportToPDF ,queryMaterial} from '@/api/planManagement/material'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          if (!err) {
            ...
            for(let item in values.details) {
                if(item['total_with_tax']>9999999999999999){
            ...
            if (type === 'submit') {
              ...
              return handleMaterial(values, 'create').then(res => {

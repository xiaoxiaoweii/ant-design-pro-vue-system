  ...
  <div>
    ...
    >
      <detail-list>
        ...
        <detail-list-item term="类型">{{ recording.reduce_type }}</detail-list-item>
...
import { handlePurchase, submitRequire,queryone,queryType } from '@/api/planManagement/disposal'
...
export default {
  ...
  data () {
    return {
      ...
      detailFields: ['name','specification','original_fee','residual_fee','amortize_fee','supplier_name','remark'],
      sumData:[{
        all_total: '合计',
        specification1: '',
        original_fee1: '',
      ...
      columnsSum: [
        ...
        {
          title: '原值（元）',
          dataIndex: 'original_fee1',
          key: 'original_fee1',
          width: '150px',
          align: 'center',
          scopedSlots: { customRender: 'original_fee1' }
        },
        {
          title: '已摊销金额',
          dataIndex: 'residual_fee1',
          key: 'residual_fee1',
          width: '150px',
          align: 'center',
          scopedSlots: { customRender: 'residual_fee1' }
        },
        {
          title: '净残值（元）',
          dataIndex: 'amortize_fee1',
          key: 'amortize_fee1',
          width: '150px',
          align: 'center',
          scopedSlots: { customRender: 'amortize_fee1' }
        },
        {
          title: '报废原因',
          dataIndex: 'supplier_name1',
          key: 'supplier_name1',
          width: '150px',
          align: 'center',
          scopedSlots: { customRender: 'supplier_name1' }
        },
        {
          title: '备注',
          dataIndex: 'remark1',
  ...
  computed: {
    ...
    getSum () {
      ...
      for (let item of this.detailData) {
        count_sum += item.original_fee*1
        count_tax += item.residual_fee*1
        count_fee += item.amortize_fee*1
      }
      this.sumData[0].original_fee1 = count_sum
      this.sumData[0].residual_fee1= count_tax
      this.sumData[0].amortize_fee1 =count_fee
      return this.sumData
    }
  },
  mounted () {
      ...
      if (data.id) {
        queryone ({ id: data.id, menu_id: 4 }).then(res => {
          this.detailData = res.responseObject.detail.map(d => {
            d.residual_fee=d.residual_fee.toString()
            d.amortize_fee=d.amortize_fee.toString()
            d.original_fee=d.original_fee.toString()
            d.name=d.name
            d.editable = false
            d.isNew = false

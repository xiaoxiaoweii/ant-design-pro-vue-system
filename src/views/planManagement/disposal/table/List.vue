<template>
  <div>
    ...
    >
      ...
      <ellipsis slot="remark" slot-scope="text" :length="40" tooltip>
        {{ text }}
      </ellipsis>
      <span
        slot="status"
...
import { handleDisposal, exportToExcel, exportToPDF } from '@/api/planManagement/disposal'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return handleDisposal(Object.assign(parameter, this.queryParam,this.orgparam), 'retrieve').then(res => {
  ...
  watch: {
    $route(to, from) {
      if (from.path === '/planManagement/disposalsNew' || from.path === '/planManagement/disposalsEdit') {
        this.refresh(true)
      }
    }
  },
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        if (this.selectedRows.length === 0 || this.selectedRows.length > 1) return this.noSelect('请选择一条记录进行编辑')
         ...
         this.$router.push({name:'disposalsEdit',params:this.selectedRows[0]})   
      } else {
        return this.$router.push({ name: 'disposalsNew',params:{type:1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleDisposal({ details: arrkeys,menu_id:95}, 'delete').then(
            res => {
    ...
    submit() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleDisposal({ details: arrkeys, flag: 0, menu_id: 95 }, 'submit').then(res => {
            if (res.status==='1') {
              that.noSelect('提交失败')
              that.refresh(true)
              return
    ...
    cancel() {
      ...
      // if(this.nickname()!==this.selectedRows[0].creator_user_name) return
      let arrkeys = []
      Newarr.map(d => arrkeys.push(d.id))
      this.$confirm({
        title: '提示',
        ...
        onOk() {
          return handleDisposal({ details: arrkeys, menu_id: 95 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

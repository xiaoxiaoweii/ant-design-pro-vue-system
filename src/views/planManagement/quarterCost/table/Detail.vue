      ...
      >
        <template v-for="(col, i) in detailFields" :slot="col" slot-scope="text,record">
          ...
          <a-input
            ...
            :key="col"
            v-if=" inputFields.includes(col)&&recording.type!='read'"
            style="margin: -5px 0"
            :value="text"
            :maxlength="1024"
...
import { handleApprove, handleRefuse,exportToExcel,exportToPDF ,getDeviceList} from '@/api/planManagement/configuration'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  data() {
    return {
      ...
      detailData: [],
      columnsTitle: [
         '材料名称', '规格型号', '单位', '数量', '使用对象', '备注','租赁','采购','调拨',
      ],
      columns: [
        {
          title: '材料名称',
  ...
  methods: {
    ...
    async loadEditInfo(data) {
      ...
      if (this.recording.process_inst_id) {
          handleQueryApprover({ menu_id: 17, process_inst_id: this.recording.process_inst_id }).then(res => {

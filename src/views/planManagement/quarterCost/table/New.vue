...
export default {
  ...
  data () {
    return {
      ...
      selectEquData: [],
      selectFields: [],
      inputFields: ['remark', 'budget_item'],
      datePickerFields: [],
      numberFields: ['accumulative_debt','new_debt','total_number'],
      popconfirmFields: [],
      popconfirmFields2: [],
      memberLoading: false,
      loading: false,
      ...
      // details table
      detailData: [],
      columnsTitle: ['预算科目', '累计欠款金额', '本期新增金额', '总金额', '备注'],
      columns: [
        {
          title: '序号',
          dataIndex: 'code',
        ...
        {
          title: '累计欠款金额',
          dataIndex: 'accumulative_debt',
          align: 'center',
          width: '200px',
          scopedSlots: { customRender: 'accumulative_debt' }
        },
        {
          title: '本期新增金额',
          dataIndex: 'new_debt',
          align: 'center',
          width: '120px',
          scopedSlots: { customRender: 'new_debt' }
        },
        {
          title: '总金额',
      ...
      detailFields: ['budget_item', 'accumulative_debt', 'new_debt', 'total_number', 'remark'],
      attachColumns: [
        {
          title: '附件名称',
          dataIndex: 'name',
  ...
  watch: {
    $route (to, from) {
      ...
      if (from.path === '/equipment/quarterCost') {
        this.isrequired = true
      }
    }
  },
  ...
  methods: {
    ...
    newDevice () {
      ...
      let length = this.detailData.length
      if (length > 1 && !this.detailData[this.detailData.length - 2].budget_item)
        return this.noSelect('请填完上一条数据')
      if(this.detailData.length>1) {
        this.detailData = this.detailData.slice(0,this.detailData.length-1)
    ...
    handleChange (value, key, column) {
      ...
      let arr1 = this.detailData.slice(0,this.detailData.length-1)
      let heji = this.detailData[this.detailData.length-1]
      if(column === 'accumulative_debt') {
        console.log("===",arr1)
        heji.accumulative_debt=''
        arr1.map(d=>{
          if(d.accumulative_debt) {
            heji.accumulative_debt += parseInt(d.accumulative_debt)
          }
        })

...
import { handleConfiguration, exportToExcel, exportToPDF, getDeviceList } from '@/api/planManagement/configuration'
...
export default {
  ...
  watch: {
    $route(to, from) {
      if (to.path === '/planManagement/configurationEdit') {
        this.params = this.$route.params
        this.$nextTick(() => {
          this.loadEditInfo(this.params)
          this.isrequired = true
  ...
  computed: {
    ...
    demo() {
      return [
        ...
        {
          title: '设备编码',
          dataIndex: 'material_category_code',
          width: '140px',
          scopedSlots: {
            filterDropdown: 'filterDropdown',
            filterIcon: 'filterIcon',
          ...
          onFilter: (value, record) => record.material_category_code.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
  ...
  methods: {
    ...
    async loadEditInfo(data) {
      ...
      if (data.id) {
        await getDeviceList({ id: data.id, menu_id: 15 }).then(res => {
    ...
    handleOk() {
      ...
      this.detailData = this.detailData.map((x, i) => {
        if (x.key === this.handleData.key) {
          x.material_name = arr[0].material_name
          x.specification = arr[0].material_model
          x.unit = arr[0].unit_name
        ...
        validateFields((err, values) => {
          if (!err) {
            ...
            if (type === 'submit') {
              ...
              return handleConfiguration(values, 'update').then(res => {

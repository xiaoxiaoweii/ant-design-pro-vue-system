<template>
  <div>
    <!-- 基本购置单信息 -->
    <a-card class="card" title="设备出租申请单" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8" >
    ...
    >
      <div class="attachment" v-if="activeTabKey === 'picture'">
        <a-upload-dragger
          ...
          :data="{ model: mod, type: 0 ,menu_id:23}"
        >
...
import { handleRental, exportToExcel,exportToPDF,getDeviceList } from '@/api/planManagement/rental'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  data() {
    return {
      ...
      datePickerFields: [],
      numberFields: [ 'num'],
      numberFields2:['date_num','price_without_tax'],
      ...
      columns: [
        ...
        },
        {
          ...
          align: 'center',
          width: '160px',
          scopedSlots: { customRender: 'date_num' }
        },
        {
          title: '税率（%）',
          dataIndex: 'tax_rate',
  ...
  watch: {
    $route(to, from) {
      if (to.path === '/planManagement/rentalEdit') {
  ...
  methods: {
    ...
    handleChange(value, key, column) {
      ...
      if (target) {
        ...
        target.sum_tax_fee = isNaN((target.price_without_tax * target.num*target.date_num).toFixed(2))?'':(target.price_without_tax * target.num*target.date_num).toFixed(2)
            ...
            if (type === 'submit') {
              ...
              this.$confirm({
                ...
                onOk() {
                  ...
                  return handleRental(values, 'submit').then(res => {
              ...
              return handleRental(values, 'update').then(res => {

...
import { handlePurchase, submitRequire, getDeviceList ,handleApprove,handleRefuse} from '@/api/planManagement/roadvehicle'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: '扣车时间',
          dataIndex: 'buckle_date',
          align: 'center',
          width: '150px',
          scopedSlots: { customRender: 'buckle_date' }
        },
        {
          title: '归还时间',
    ...
    async loadEditInfo(data) {
      ...
      if (data.id) {
        ...
        if (this.recording.process_inst_id) {
          handleQueryApprover({ menu_id: 18, process_inst_id: this.recording.process_inst_id }).then(res => {

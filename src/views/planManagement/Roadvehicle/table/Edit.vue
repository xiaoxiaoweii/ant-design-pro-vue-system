<template>
  <div>
    <!-- 基本购置单信息 -->
    <a-card class="card" title="路用车需求计划单" :bordered="false">
      <a-form :form="form">
        <a-row :gutter="16">
          <a-col :span="8" >
      ...
      >
        <template v-for="(col, i) in detailFields" :slot="col" slot-scope="text, record">
          ...
          <a-input-number
            ...
            :min="0"
            :precision="0.1"
            :max="col=='sum'?999999999:99999999"
            :placeholder="columnsTitle[i]"
    ...
    <a-modal class="modal-box" title="请选择" v-model="visible" width="40%"
    @cancel="handleCancel"
    ...
    >
      <div class="attachment" v-if="activeTabKey === 'picture'">
        <a-upload-dragger
          ...
          :data="{ model: mod, type: 0 ,menu_id:18}"
        >
...
import { handleRoadvehicle, exportToExcel,exportToPDF,getDeviceList } from '@/api/planManagement/roadvehicle'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  data() {
    return {
      ...
      selectEquData: [],
      selectFields: ['car_demo'],
      inputFields: ['buckle_bureau','buckle_site','use_site','usage','remark'],
      datePickerFields: ['buckle_date1','return_date1'],
      numberFields: ['sum'],
      popconfirmFields: [],
      memberLoading: false,
      loading: false,
      ...
      columns: [
        ...
        },
        {
          ...
          align: 'center',
          width: '150px',
          scopedSlots: { customRender: 'buckle_bureau' }
        },
        {
          title: '扣车站',
          dataIndex: 'buckle_site',
          align: 'center',
          width: '120px',
          scopedSlots: { customRender: 'buckle_site' }
        },
        {
          title: '车种',
          dataIndex:'car_demo',
          width:'120px',
          align: 'center',
          scopedSlots: { customRender: 'car_demo' }
        },
        {
          title: '数量',
          dataIndex: 'sum',
        ...
        {
          title: '扣车时间',
          dataIndex: 'buckle_date1',
          align: 'center',
          width: '150px',
          scopedSlots: { customRender: 'buckle_date1' }
        },
        {
          title: '归还时间',
          dataIndex: 'return_date1',
          align: 'center',
          width: '150px',
          scopedSlots: { customRender: 'return_date1' }
        },
        {
          title: '使用区间',
          dataIndex: 'use_site',
    ...
    demo() {
      ...
      return [
        ...
        },
        {
          ...
          onFilter: (value, record) => record.equipment_code.toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
            if (visible) {
              setTimeout(() => {
                this.searchInput.focus()
  ...
  watch: {
    $route(to, from) {
      if (to.path === '/planManagement/roadvehicleEdit') {
        this.params=this.$route.params        
        this.$nextTick(() => {
          this.loadEditInfo(this.params)
          this.isrequired = true
  ...
  mounted() {
   getdictionary({ dic_type_id: 1028 }).then(res => {
      selectData.car_demo = res.responseList
    }) 
  ...
  methods: {
    ...
    handleEnameOk (val) {
      ...
      this.detailData = this.detailData.map((x, i) => {
        if (x.key === this.handleTreeData.key) {
          x.use_org_name = this.selectNode
        }
        return x
      })
      this.input2 = ''
   ...
   async loadEditInfo(data) {
      ...
      if (data.id) {
        await getDeviceList({ id: data.id, menu_id: 18 }).then(res => {
          this.detailData = res.responseObject.details.map((d,i) => {
            ...
            d.buckle_date && (d.buckle_date1 = moment(d.buckle_date))
            d.return_date && (d.return_date1 = moment(d.return_date))
            d.key = d.id
           if (d.number==0) d.number=''
          ...
          this.fileList = res.responseObject.files.map(attachment => {
            // if (attachment.type === 0) {
              ...
              attachment.fileSize = (attachment.file_name/1024).toFixed(0)
    ...
    valueChange(e) {
      ...
      this.axios({url:'/orgtree',method:'get',params:{name:this.valTree}}).then(res=>{
    ...
    handleOk() {
      ...
      this.visible = false
      const arr = this.tableData.responsePageInfo.list.filter(item => item.id === this.selectedRowKeys[0])
      ...
      this.detailData = this.detailData.map((x, i) => {
        if (x.key === this.handleData.key) {
          x.equipment_name = arr[0].equipment_name
          x.equipment_spec = arr[0].material_model
          x.equipment_unit = arr[0].unit_name
        //   x.model = arr[0].material_model
    ...
    // })
     let myres=[]
      this.axios.get('/orgtree').then(res=>{
        myres.push(res.responseObject)
        this.dicTree = myres
        this.dicTree2 = myres
      })
      this.handleTreeData.col = col
    ...
    handleGoBack() {
        ...
        this.$router.push('roadvehicleList')
    ...
    newDevice() {
      ...
      // 限制只能同时填写一条数据
      if (length > 0 && !this.detailData[this.detailData.length - 1].car_demo) return this.noSelect('请填完上一条数据')
    ...
    handleChange(value, key, column) {
      ...
      if (target) {
        target[column] = value
        if(['boxcar_num','wagoncar_num','flatcar_num','tankcar_num','other_num'].includes(column)) {
            target.sum = target.boxcar_num+target.wagoncar_num+target.flatcar_num+target.tankcar_num+target.other_num
          if(isNaN(target.sum)) target.sum = null  
        }
        this.detailData = newData
      }
    },
        ...
        validateFields((err, values) => {
          if (!err) {
            ...
            values.details = that.detailData.map((x) => {
              x.buckle_date1 && (x.buckle_date = x.buckle_date1.format('YYYY-MM-DD'))
              x.return_date1 && (x.return_date = x.return_date1.format('YYYY-MM-DD'))
              return x
            })
            ...
            if (type === 'submit') {
              ...
              this.$confirm({
                ...
                onOk() {
                  ...
                  return handleRoadvehicle(values, 'submit').then(res => {
              ...
              return handleRoadvehicle(values, 'update').then(res => {
    ...
    handleReset(clearFilters, col) {
      ...
      clearFilters()
      this.queryParam = {
      equipment_code: this.myEquipment_code
      }
      this.$refs.tables.refresh(true)
    },

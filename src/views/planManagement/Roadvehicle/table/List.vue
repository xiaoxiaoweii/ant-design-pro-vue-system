...
import { handleRoadvehicle, exportToExcel } from '@/api/planManagement/roadvehicle'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return handleRoadvehicle(Object.assign(parameter, this.queryParam,this.orgparam), 'retrieve').then(res => {
  ...
  watch: {
    $route(to, from) {
      if (from.path === '/planManagement/roadvehicleNew' || from.path === '/planManagement/roadvehicleEdit') {
        this.refresh(true)
      }
  ...
  methods: {
    ...
    handleEdit (bool) {
      if (bool) {
        if (this.selectedRows.length === 0 || this.selectedRows.length > 1) return this.noSelect('请选择一条记录进行编辑')
         ...
         this.$router.push({name:'roadvehicleEdit',params:this.selectedRows[0]})   
      } else {
        return this.$router.push({ name: 'roadvehicleNew',params:{type:1} })
      }
    },
    ...
    remove () {
      ...
      this.$confirm({
        ...
        okType: 'danger',
        onOk () {
          return handleRoadvehicle({ details: arrkeys,menu_id:18}, 'delete').then(
            res => {
    ...
    submit() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleRoadvehicle({ details: arrkeys, flag: 0, menu_id: 18 }, 'submit').then(res => {
            // res.status === '0'
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
    ...
    cancel() {
      ...
      this.$confirm({
        ...
        onOk() {
          return handleRoadvehicle({ details: arrkeys, menu_id: 18 }, 'cancel').then(res => {
            if (res.responseList.length !== 0) {
              const subdata = []
              for (var i = 0; i < that.selectedRows.length; i++) {
                for (var j = 0; j < res.responseList.length; j++) {

...
import { handleRoadvehicle, exportToExcel,exportToPDF } from '@/api/planManagement/roadvehicle'
import {
  uploadUrl,
  getAttachments,
  downloadAttachment,
...
export default {
  ...
  watch: {
    $route(to, from) {
      ...
      if (from.path === '/planManagement/requireList') {
        this.isrequired = true
      }
    }
  },
      ...
      setTimeout(() => {
        validateFields((err, values) => {
          if (!err) {
            ...
            if (type === 'submit') {
              console.log('000',values.details)
              ...
              return handleRoadvehicle(values, 'create').then(res => {
...
.ant-ant-select-enabled>div.ant-select-selection{
